...
حفظ سند صرف بدل أجازة يقوم بثلاثة أمور في آنٍ واحد. أولاً، وتماماً كسند الأجازة العادي، يستهلك عدد أيام `مدة الأجازة` من رصيد الموظف لهذا النوع من الأجازة. ثانياً — وعلى خلاف سند الأجازة العادي — يُنشئ **طلب أعمال** (Business Request) يُرحّل قيداً محاسبياً فعلياً: سطر مدين وسطر دائن، كلاهما بقيمة **إجمالي البدل**، إلى أي حسابات مُعرَّفة على توجيه سند صرف بدل أجازة (الجانبان يُسميان هناك **مدين 2** / **دائن 2**). تعديل سند تعويض سبق ترحيله يعيد إصدار هذا القيد بالقيمة الجديدة؛ وحذفه يعكس القيد ويعيد الأيام إلى الرصيد. وقيمة البدل تُصرف بهذا القيد وحده — ولا تُضاف كسطر في سند راتب الموظف، فلا تبحث عنها هناك.

ثالثاً — وهذا الأثر يسهل أن يفوتك لأن الشاشة لا تُظهره — يكتب السند في بطاقة الموظف نفسه حقل **تاريخ المباشرة بعد أخر اجازة** (Commencement Date After Holiday) ويضع فيه **تاريخ العودة** المدوّن في السند. وهو نفس الحقل الذي يملؤه رجوع حقيقي من أجازة سنوية، ويملؤه سند صرف البدل كلما كان نوع الأجازة المصروف بدلها يحمل **تصنيف الاجازة** السنوي — رغم أن الموظف لم يغادر عمله أصلاً. وحذف سند صرف البدل يعيد هذا التاريخ إلى ما تركه آخر سند مباشرة سابق للموظف.

::: warning صرف بدل الأجازة قد يقتطع جزءاً من فترة الراتب
يبقى هذا الأثر الثالث بلا ضرر ما لم يكن خيار **بداية سندالراتب يحسب من تاريخ اخر مباشرة** (Salary Worth From Last Work Start) مفعّلاً في [إعدادات الموارد البشرية](../setup/hr-configuration.md). فمع تفعيل هذا الخيار يبدأ استحقاق سند الراتب من تاريخ آخر مباشرة للموظف بدل بداية الفترة — وعليه فإن سند صرف بدل أجازة مؤرخاً في اليوم الرابع عشر من الشهر ينقل بداية الراتب إلى اليوم الرابع عشر. عندئذ يُولَّد راتب الشهر من ١٤ إلى ٣٠ فقط وعليه علامة **فترة غير مكتملة (جزئية)**، ولا يُصرف شيء عن الفترة من ١ إلى ١٣: لا يوجد سند راتب ثانٍ يغطي الأيام التي تخطّتها بداية الفترة.

والخيار هنا يعمل تماماً كما يقول — فهو موضوع لمن عاد من أجازة بلا راتب في منتصف الشهر — لكن صرف البدل ليس غياباً، لذا نادراً ما تكون النتيجة هي المطلوبة في الرواتب. فإذا نقصت رواتب الموظفين في الشهر نفسه الذي صرفوا فيه بدل أجازة، فابدأ البحث من هذين الأمرين. وإيقاف الخيار يعيد الفترة كاملة، ثم يجب إعادة توليد سند راتب ذلك الشهر لأن السند الصادر بالفعل لا يعيد حساب نفسه.
:::
...
|---|---|---|
| «يجب ان تكون مدة الأجازة أكبر من الصفر» | **مدة الأجازة** في سند صرف بدل الأجازة صفر أو أقل، فلا توجد أيام تُصرف. | أدخل عدد الأيام المطلوب صرف بدلها. |
| «يرجي تحديد تصنيف الاجازة فى نوع الاجازة {0}» | نوع الأجازة المختار في السند ليس له **تصنيف الأجازة** في بطاقته، والتصنيف هو ما يحدد كيفية تقييم البدل. | افتح نوع الأجازة وحدّد تصنيفه، ثم أعد حفظ السند. |
| «التاريخ الفعلي يجب ان يكون مساوي لبداية الي سنة الرواتب({0})» | سند ترحيل الأجازات ينقل الأرصدة إلى سنة موارد بشرية جديدة، فيجب أن يكون مؤرَّخًا بأول يوم فيها — و**التاريخ الفعلي** ليس كذلك. | اجعل التاريخ الفعلي مساويًا لتاريخ بداية **إلى سنة** الظاهر في الرسالة. |
| «لا يمكن الترحيل لسنة قديمة» | **إلى سنة** تبدأ في تاريخ **من سنة** أو قبله، فيصبح الترحيل إلى الخلف. | رحّل إلى الأمام: من السنة المنتهية إلى السنة التي تبدأ. |
| «لايمكن تعديل الرصيد للموظف {0} تم تعدبل الرصيد فى مستند بتاريخ لاحق طبقا لسند  {1}» | يوجد سند تعديل رصيد أجازة لاحق في السنة نفسها عدّل رصيد هذا الموظف، فتغيير الرقم الأسبق يترك اللاحق مبنيًا على رقم لم يعد قائمًا. | عدّل أحدث سند تعديل رصيد، أو ألغِه أولًا ثم صحّح هذا المستند. |
...
- **[إعدادات الموارد البشرية](../setup/hr-configuration.md)** — موضع خيار «بداية سندالراتب يحسب من تاريخ اخر مباشرة»، وهو الذي يحدد ما إذا كان تاريخ المباشرة الذي يكتبه سند صرف البدل سيصل إلى سند الراتب أم لا.
