...
## الخطأ: مجلد النسخ الاحتياطي غير محدد أو غير موجود أو لا يحوي نسخة حديثة

ثلاث رسائل مترابطة تأتي كلها من فحص واحد — الفحص الذي يتأكد من وجود نسخة احتياطية حديثة لقاعدة البيانات فعلاً:
...
وكل واحدة منها ترتبط مباشرة بالإعداد الذي تخصه، فالضغط على الرسالة يفتح الإعدادات العامة وحقل **مجلد النسخ الاحتياطي** في بؤرة التركيز:
<GlobalConfigOption option-code="value.info.backupFolder" />

### عمّ يبحث الفحص

يقرأ النظام المجلد الذي تسميه في **مجلد النسخ الاحتياطي**، وحتى مستويين من المجلدات الفرعية تحته، ويبحث عن ملف منته بـ `.bak` أو `.dbak` يحمل اسمه تاريخ اليوم أو الأمس بصيغة `YYYYMMDD`، أو كان آخر تعديل عليه منذ بداية يوم أمس. فإن وجد واحداً فلا رسالة. ويعمل الفحص عند بدء تشغيل الخادم ويتكرر كل ساعة.

### خطوات الحل

1. **الرسالة الأولى** — الحقل فارغ. افتح صفحة **المرفقات والتخزين** في الإعدادات العامة واملأ **مجلد النسخ الاحتياطي** بالمسار الذي تكتب فيه مهمة النسخ الاحتياطي.
2. **الرسالة الثانية** — المسار مملوء لكن الخادم لا يصل إليه. وتذكّر أن الذي يحلّ المسار هو خادم التطبيق لا جهازك: راجع الكتابة، وإن كانت مشاركة شبكية فتأكد أن حساب ويندوز الذي يعمل به Tomcat يستطيع فتحها فعلاً.
3. **الرسالة الثالثة** — المجلد متاح لكنه خالٍ من الملفات الحديثة. وهذه أهم الرسائل: فهي تعني غالباً أن مهمة النسخ الاحتياطي نفسها توقفت أو بدأت تفشل. راجع خطة الصيانة في SQL Server (أو الأداة التي تأخذ نسخك) وانظر في سجل تشغيلها بحثاً عن الأخطاء، ثم تأكد أنها تكتب في هذا المجلد بامتداد `.bak` أو `.dbak`.

::: danger لا تُسكت هذه الرسالة باستخفاف
الرسالة الثالثة هي بعينها التحذير الذي تتمناه يوم يتعطل الخادم. فإن ظهرت، فعاملها كنسخة احتياطية معطوبة حتى تثبت العكس — ولا تعطّل الفحص لتختفي.
:::

### حين لا يستطيع النظام رؤية نسخك الاحتياطية

بعض المواقع تأخذ نسخها بطريقة لا يراها الخادم — لقطة على مستوى وحدة التخزين، أو قاعدة بيانات سحابية مُدارة، أو وكيل ينقل الملفات مباشرة خارج الجهاز. وهناك لا يجد الفحص شيئاً ويرفع إنذاراً كاذباً دائماً، فعطّله بـ:
<GlobalConfigOption option-code="value.info.doNotCheckForBackupExistence" />

وتجد الشرح الكامل للإعدادين في [المرفقات والتخزين](/ar/platform/global-config/global-config-attachments).

::: tip ملحوظة هامة
يرجى أن تقوم بعمل Refresh Critical Errors من صفحة أدوات النظام Utilities بعد تصحيح هذا الخطأ، أو انتظر التحديث الذي يجري كل ساعة.
:::
