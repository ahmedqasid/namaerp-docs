...
**معادلة الحضور والانصراف** هي الطريقة التي تُعلِّم بها ناما كيف تقرأ ملف ماكينة بعينها. وهي سطر قصير من الرموز المسبوقة بعلامة `#` يصف، عموداً بعمود، ما يحتويه الملف المُصدَّر فعلياً. وبمجرد وجود المعادلة، يصبح استيراد ملف تلك الماكينة عملية من نقرتين طوال عمرها.

::: tip هناك أيضاً مسار مباشر ومؤتمت
هذه الصفحة تتحدث عن استيراد **ملف صدَّرته الماكينة**. وناما تستطيع أيضاً الاتصال ببعض الماكينات (أو بقواعد بياناتها) مباشرة وسحب البصمات وفق جدول زمني، بلا ملف وبلا معادلة — راجع [ماكينات الحضور](attendance/attendance-machines.md). الآليتان مستقلتان؛ والماكينة الواحدة عادةً لا تحتاج إلا إحداهما.
:::
...
قبل تفسير أي تواريخ أو أوقات، على ناما أن تقسّم الملف إلى سطور وأعمدة. والرموز التالية تتحكم في هذه الخطوة.
...
عندما يكون المرفق بصيغة `.xlsx`، تقرأ ناما الورقة خلية بخلية وتعيد ربط كل سطر باستخدام **الفواصل** قبل تطبيق المعادلة. لذا اترك `#sep` على قيمته الافتراضية مع ملفات إكسل. فتحديد أي قيمة أخرى سيُفسد الاستيراد — كما أن الخلية التي يحتوي نصها على فاصلة ستُقسَّم إلى عمودين.
:::
...
بعض الماكينات تكتب اليوم كله في سطر واحد: حضور وانصراف الوردية الصباحية، ثم حضور وانصراف الوردية المسائية، جنباً إلى جنب. وناما تخزّن سطراً واحداً لكل وردية، لذا يجب تحويل تلك الأعمدة الإضافية إلى سطور إضافية.
...
لملفات إكسل فقط. بدونه، تفسّر ناما الخلايا المنسقة كتواريخ بنفسها وتسلّم المعادلة قيمة موحدة. ومعه، يُستخدم النص المعروض في الخلية تماماً كما تُظهره الورقة. استخدمه عندما ينتج استيراد إكسل تواريخ مُزاحة أو مُعاد تنسيقها أو ترفض مطابقة نمطك.

## وصف الأعمدة (Describing the columns)

### `#empid` — الموظف

**مطلوب عملياً.** هذا هو كود الموظف **على الماكينة**، والذي تطابقه ناما مع حقل **كود آلة الحضور والإنصراف** (Attendance Machine Code) في بطاقة الموظف.
...
تتحقق ناما مما إذا كان العمود يطابق العلامة الواقعة **قبل** الشرطة. وأي شيء آخر — علامة الانصراف، أو قيمة غير متوقعة، أو خلية فارغة — يُعامَل على أنه **انصراف**. والمقارنة حساسة لحالة الأحرف، ولا يمكن أن تحتوي علامة الحضور نفسها على شرطة.
...
هذا هو الجزء الذي يحدد كم سطراً ستتحول إليه قراءات الماكينة، وهو مصدر معظم مشكلات الاستيراد. فالماكينة التي تكتب قراءة واحدة في كل سطر تعطيك كومة من الأوقات؛ وعلى ناما أن تستنتج أيها حضور، وأيها الانصراف المقابل له، وأيها ينتمي معاً إلى سطر حضور واحد.

اختر **واحداً** من الأنماط التالية.

### الوضع الافتراضي: حضور يتبعه انصراف

بلا أي رمز مطابقة على الإطلاق، تطابق ناما الحضور مع الانصراف الذي يليه مباشرة **في نفس التاريخ**، وتترك الانصراف غير المطابَق كسطر فيه وقت انصراف فقط. وهذا مطابق تماماً لكتابة `#exact`.
...
إذا كانت معادلتك تستخدم `#indate` أو `#intime` أو `#indatetime`، فإن الملف يخبر ناما أن كل سطر **يحتوي بالفعل على حضور وانصراف كاملين** — فلا حاجة إلى أي مطابقة ويُتجاهَل كل نمط من أنماط المطابقة.

::: warning السطر يحتاج إلى تاريخ انصراف كما يحتاج إلى وقت انصراف
عند استخدام أعمدة الحضور/الانصراف يجب أن تعطي ناما أيضاً مكاناً تقرأ منه **تاريخ** الانصراف: `#outdate` أو `#outdatetime` أو `#date` مشترك يغطي الاثنين. فالمعادلة التي تحتوي على `#indate` و `#intime` و `#outtime` بلا تاريخ انصراف يفشل استيرادها برسالة *"Could not find parameter outdatetime"*.
:::
