# وحدة BI — لوحات المعلومات والتحليلات (BI Module — Dashboards & Analytics) {#BI-Module----Dashboards---Analytics}
...
## ما الذي ستحصل عليه {#What-You-Get}
...
## أنواع المخططات (Chart Types) {#Chart-Types}
...
### مخططات الأعمدة (Bar Charts) — 10 قوالب {#Bar-Charts--10-templates-}
...
### مخططات الخطوط (Line Charts) — 6 قوالب {#Line-Charts--6-templates-}
...
### مخططات المساحة (Area Charts) — 4 قوالب {#Area-Charts--4-templates-}
مساحة أساسية، مساحة مكدّسة، مساحة بتعبئة متدرجة، ومساحة نطاقية.

### مخططات الدائرة والحلقة (Pie & Donut Charts) — 6 قوالب {#Pie---Donut-Charts--6-templates-}
...
### الحالة B — محرر المخطط (Chart Editor) {#State-B----Chart-Editor}
...
#### تبويب ربط البيانات (Data Mapping Tab) {#Data-Mapping-Tab}
...
#### تبويب النمط (Style Tab) {#Style-Tab}
...
#### محرر JSON المتقدم (Advanced JSON Editor) {#Advanced-JSON-Editor}
...
#### تبويب Click & Links {#Click---Links-Tab}

هيّئ ما يحدث عندما يتفاعل المستخدمون مع المخطط — مشروح بالتفصيل في قسمَي [الفلاتر المتقاطعة](#Cross-Filtering) و[الروابط](#Links----Navigate-to-Records) أدناه.

#### تبويب Drill-Down {#Drill-Down-Tab}

هيّئ أهداف drill-down — مشروح في قسم [Drill-Down](#Drill-Down) أدناه.

---

## لوحات المعلومات والتخطيط (Dashboards & Layout) {#Dashboards---Layout}
...
- **Tabbed** — حاوية تحمل *لوحات معلومات أخرى* كتبويبات قابلة للتبديل، بدلاً من حمل الـ widgets مباشرة. راجع [لوحات المعلومات ذات التبويبات (Tabbed Dashboards)](#Tabbed-Dashboards) أدناه.

### تعديل التخطيط (Editing the Layout) {#Editing-the-Layout}
...
### التحديث التلقائي (Auto-Refresh) {#Auto-Refresh}
...
### أنواع الـ Widgets على لوحة المعلومات {#Widget-Types-on-a-Dashboard}
...
## لوحات المعلومات ذات التبويبات (Tabbed Dashboards) {#Tabbed-Dashboards}
...
### قصة إعادة الاستخدام {#The-Reuse-Story}
...
### الفلاتر المتقاطعة عبر التبويبات {#Cross-Filters-Across-Tabs}
...
### الأداء — التحميل الكسول لكل تبويب {#Performance----Lazy-Loading-Per-Tab}
...
### الإشارات المرجعية والمشاركة {#Bookmarking---Sharing}
...
### ما الذي يختلف في تعديل لوحات Tabbed {#What-s-Different-About-Editing-Tabbed-Dashboards}
...
## الفلاتر المتقاطعة (Cross-Filtering) {#Cross-Filtering}
...
### شريط الفلاتر (The Filter Bar) {#The-Filter-Bar}
...
### أنواع الفلاتر (Filter Types) {#Filter-Types}
...
### التغذية الراجعة البصرية (Visual Feedback) {#Visual-Feedback}
...
### Widget Drill-Down (مخطط → مخطط) {#Widget-Drill-Down--Chart----Chart-}
...
### Dashboard Drill-Down (مخطط → لوحة معلومات) {#Dashboard-Drill-Down--Chart----Dashboard-}
...
### Dimension Drill-By (نفس المخطط، بُعد أعمق) {#Dimension-Drill-By--Same-Chart--Deeper-Dimension-}
...
### Single-Click Drill-Down {#Single-Click-Drill-Down}
...
### حوار Drill-Down {#The-Drill-Down-Dialog}
...
## الروابط — التنقل إلى السجلات (Links — Navigate to Records) {#Links----Navigate-to-Records}

أحياناً تريد الضغط على نقطة بيانات في مخطط والانتقال إلى سجل ERP الفعلي خلفها. هذا ما ربط الروابط (link mappings) مخصص له.

### روابط الكيانات (Entity Links) {#Entity-Links}
...
### أين تظهر الروابط {#Where-Links-Appear}
...
## قائمة السياق عند زر الأيمن (The Right-Click Context Menu) {#The-Right-Click-Context-Menu}
...
## ملخص تفاعلات المخطط (Chart Interactions Summary) {#Chart-Interactions-Summary}
...
## Widgets الجداول (Table Widgets) {#Table-Widgets}
...
## Enhanced Table Widget {#Enhanced-Table-Widget}
...
### ما يمكنك فعله لكل عمود {#What-You-Can-Do-Per-Column}
...
### تنسيق مستوى الصف (Row-Level Styling) {#Row-Level-Styling}
...
### التفاعلات — نفس البقية {#Interactions----Same-As-The-Rest}
...
### ملاءمات المسؤول (Admin Conveniences) {#Admin-Conveniences}
...
للإعداد الأكثر تقدماً — تحديداً مجموعات الأعمدة، وخيارات كل مصيّر مثل bar min/max أو تعيينات الأيقونات، وDSL قواعد التنسيق الشرطي الكاملة — استخدم تبويبة **Raw JSON**. المرجع التقني [§14](./bi-module-technical-reference.md#14-EnhancedTable----JSON-Driven-Grid) يحتوي الـ schema الكاملة.

### متى تستخدم أيًّا منهما {#When-To-Use-Which}
...
### ترقية جدول موجود (Upgrading An Existing Table) {#Upgrading-An-Existing-Table}
...
### مصدر بيانات Wizard {#Wizard-Data-Source}
...
## تنسيق الأرقام (Number Formatting) {#Number-Formatting}
...
## مقارنة الفترات (Period Comparison) {#Period-Comparison}
...
### وضعان للمقارنة {#Two-Modes-of-Comparison}

#### الوضع A — عمود تسمية الفترة (للمخططات الاتجاهية) {#Mode-A----Period-Label-Column--for-trend-charts-}
...
#### الوضع B — تقسيم العمود (لمخططات الأعمدة والسلاسل الصريحة) {#Mode-B----Column-Split--for-bar-charts-and-explicit-series-}
...
### معاملات الاستعلام (Query Parameters) {#Query-Parameters}
...
## إنشاء لوحات المعلومات بالذكاء الاصطناعي (Creating Dashboards with AI) {#Creating-Dashboards-with-AI}
