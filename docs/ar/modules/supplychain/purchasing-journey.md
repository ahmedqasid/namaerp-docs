...
مستندات الشراء تتبع العادتين نفسيهما اللتين في جانب المبيعات: زر **تجميع** يلمّ ما هو قائم بالفعل، وزر **تطبيق** يحوّل ما جُمِع إلى سطور مسعَّرة.

**في فاتورة المشتريات:**

- **تجميع** (Collect) — يسأل عن **تاريخ من** و**تاريخ إلى**، ويملأ جدول المستندات المخزنية بأوامر الاستلام التي أُخذت من هذا المورد في نفس المخزن، وسُجّلت مرة على الأقل، وليس عليها علامة *عدم التجميع في الفواتير*، ولم تُفوتر بعد. أما جهة الاتصال والذمة فيضيّقان البحث أيضًا، لكن فقط إذا كان التوجيه مفعّلًا فيه *اعتبار جهة الاتصال عند تجميع السندات المخزنية في الفواتير* / *اعتبار الذمة عند تجميع السندات المخزنية في الفواتير*.
- **تطبيق** (Apply Receipts) — يحوّل تلك الاستلامات إلى سطور فاتورة مسعَّرة، بمصنِّفات أسعار الفاتورة وإعدادات الضريبة والتصنيف والتوجيه.
- **إنشاء سند مخزني** (Create Inventory Doc.) — لفاتورة أُدخلت يدويًا، ينشئ أمر الاستلام ويفتحه.
- **إنشاء سند صرف** (Generate payment voucher) — يجب حفظ الفاتورة أولًا؛ ينشئ سند صرف بكامل المتبقي على المورد ويفتحه كسجل جديد. و**إنشاء سند صرف للسطور المختارة** (Generate Payment Voucher For Selected Payments) يفعل ذلك لسطور الأقساط التي أشّرت عليها ولها متبقٍّ فقط. أما **تجميع سندات الصرف** (Collect Payment Vouchers) فيجلب السندات القائمة إلى الفاتورة.
- **إنشاء الدفعات** (GeneratePayments) — يقسّم المتبقي إلى جدول أقساط، ويسأل عن عدد الدفعات والمدة بينها ووحدتها وتاريخ البدء وفترة السماح ويوم الأسبوع وقيم الدفعة المقدمة والأولى والثانية والأخيرة وطريقة التقريب.

**في أمر الشراء والفاتورة المبدئية للشراء:**

- **نسخ السطور إلى جدول الاستلامات** (Copy Lines To Receipt Lines) — ينسخ سطور تفاصيل الأمر إلى جدول الاستلامات به، فتبدأ التسليمات المتوقعة مما طُلب بدل إعادة كتابته.
- **إنشاء الدفعات** (GeneratePayments) — نفس نافذة الجدولة التي في الفاتورة. وهي موجودة كذلك في عرض سعر المشتريات وطلب عرض سعر المشتريات.

**في طلب الشراء المجمَّع:**

- **تجميع الطلبات** (Collect Requests) — يلمّ طلبات الأصناف المفتوحة الواقعة داخل النطاقات التي حددتها في الرأس (التواريخ، وطلب الصنف، وطالب الشراء، والمورد، والشركة، والمجموعة التحليلية، والفرع، والقطاع، والإدارة)، ويملأ جدول الطلبات وسطوره.
- **إنشاء عروض أسعار مشتريات للموردين المختارين** (Generate Purchase Quotations For Selected Suppliers) — يجب حفظ المستند؛ يسأل هل يأخذ الكميات من **الكمية المعتمدة** في سطور الطلبات أم من **الكمية المتبقية** في التفاصيل، ثم ينشئ عرض سعر مشتريات لكل مورد مختار.
- **تحويل إلى أمر شراء** (Generate Purchase Order) — يحتاج المستند محفوظًا أيضًا، ويبني منه أمر شراء واحدًا وينقل إليه سطور التفاصيل.

**في مقارنة أسعار الشراء:**

- **تجميع عروض الأسعار الخاصة بالطلب** (Collect Quotations Based On Request) — يقرأ طلب عرض السعر المذكور في الرأس ويملأ جدول المقارنة بكل عرض ورد عليه، فتصطفّ العروض جنبًا إلى جنب.
- **إنشاء أوامر شراء لأفضل الأسعار** (Generate Purchase Orders For Best Prices) — ينشئ أوامر الشراء الناتجة عن المقارنة، أمرًا لكل مورد ظفر بأفضل سعر.

**في مردود المشتريات:**

- **تجميع** (Collect) و**تطبيق** (Apply Receipts) — نفس الثنائي، لكنه موجَّه إلى أوامر الصرف، لأن مردود المشتريات يُخرج البضاعة.
- **تجميع الدفعات من بناءاً علي** (collect Installments By From Doc) يعيد بناء جدول أقساط المردود من الأقساط غير المسدَّدة في الفاتورة المبني عليها، آخذًا قيمة المردود من أقدم قسط فصاعدًا؛ و**توزيع قيمة العائد على جميع الأقساط الغير المسددة** (Distribute Return Value On All Unpaid installments) يوزّعها بالتساوي على غير المسدَّد. وكلاهما يحتاج مستند **بناءً على**.

**في مستند تحديث مستندات الشراء:** **تطبيق التغييرات على المستندات** (Apply Changes To Purchase Documents) — يجب حفظ المستند أولًا، ثم يدفع ما يحمله من تغييرات إلى مستندات الشراء المستهدفة دفعة واحدة.

**في قائمة سعر الشراء:**

- **تجميع الأصناف** (Collect Items) — يملأ السطور من معايير تجميع الأصناف التي أدخلتها بدل إدخالها صنفًا صنفًا.
- **تحديث التفاصيل** (Update Details) — ينزل بقيم الرأس إلى كل سطر تفاصيل. ويسأل عن كل منها على حدة بنعم/لا: تاريخ من، وتاريخ إلى، والمجموعة التحليلية، والشركة، والفرع، والإدارة، والقطاع، و*تحديث العميل-المورد* (Update Customer)، وتصنيف الفاتورة، ومصنِّفات الأسعار من ١ إلى ٥. وسؤال المورد يحمل صيغته البيعية نفسها في قائمة أسعار الشراء، والإجابة بنعم تكتب مورّد الرأس في كل سطر.
- **تحديث الأسعار** (Update Prices) — يعيد حساب الأسعار عبر قواعد محدِّثات السعر في الرأس: حقل مصدر وحتى خمسة محدِّثات، تُطبَّق على كل السطور أو على المختار منها فقط. وليس في قائمة أسعار الشراء حقول وجهة تختار منها؛ فكل محدِّث يكتب في عمود **السعر** الوحيد. ويرفض العمل إن كانت القائمة بلا سطور أو لم يُملأ أي محدِّث.
- **حساب السعر من متوسط التكلفة** (calculate Price from Average Cost) — يملأ عمود **السعر** من متوسط تكلفة كل صنف بدل ملئه من حقل سعر آخر. وكما في تحديث الأسعار، ليس في قائمة أسعار الشراء حقل وجهة تختاره.
...
|---|---|---|
| «التوجيه: {0} المختار لإنشاء المستند ليس من نوع: {1}» | **طلب الشراء المجمَّع** يولّد مستندات، وتوجيه التوليد المذكور في توجيهه تابع لنوع مستند مختلف عن النوع المطلوب توليده. | في توجيه الطلب، اختر توجيه توليد تابعًا لنوع المستند المولَّد — توجيه أمر شراء لأوامر الشراء وهكذا. |
| «الدفتر: {0} المختار لإنشاء المستند ليس من نوع: {1}» | عدم التطابق نفسه في جهة الدفتر: دفتر التوليد تابع لنوع مستند آخر. | اختر دفترًا من نوع المستند المولَّد في توجيه الطلب. |
| «لايمكن إنشاء مستندات بناءا علي المستند {0}, لايوجد اي سطر به كمية في الحقل {1}» | التوليد يقرأ الكميات من حقل محدد — الكمية المتبقية في سطور الشراء أو الكمية المعتمدة في سطور الطلبات — وكل السطور هذا الحقل فيها فارغ أو صفر. والرسالة تذكر الحقل الذي قرأه. | املأ الحقل الذي تذكره الرسالة، أو غيّر الحقل الذي يولّد منه التوجيه. |
| «{0} خطأ اثناء تعديل مستند الشراء» | زر **تنفيذ التعديلات على مستندات الشراء** في مستند تعديل مستندات الشراء أخفق على أحد المستندات المستهدفة؛ والرسالة تذكر ذلك المستند ويليها السبب الحقيقي، وهي الرسائل التي رفعها المستند نفسه. | اقرأ الرسائل التي تحت هذه — فهي تابعة للمستند المذكور — وافتحه لتصحيح ما تصفه. والتنفيذ يتوقف عند أول مستند يخفق. |
