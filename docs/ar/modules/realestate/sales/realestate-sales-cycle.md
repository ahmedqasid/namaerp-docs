...
|---|---|---|---|
| عرض سعر بيع | لا | فقط إذا أُعدَّ توجيهه — ويُترك فارغًا عادةً، فلا يقيّد شيئًا | `realestate-sales` |
...
*للشركة* أو بإلغاء العقد أو حذفه — راجع
...
- لا يصل إلى دفتر الأستاذ إلا إذا أعددته لذلك عن قصد. فللعرض توجيه مستند خاص به يحمل مجموعة
  الحسابات كاملةً كما يحملها العقد، وعند اعتماده يرفع طلب قيد محاسبي تمامًا كما يفعل العقد — لكن
  ما دامت تلك الحسابات فارغة، وهي كذلك عادةً، فلا يُكتب شيء. املأها وسيقيّد العرض كأي مستند آخر،
  فاترك توجيه العرض دون إعداد ما لم تكن تقصد غير ذلك.
