...
|---|---|---|
| *Could not find any materials in planning document :{0}* — «لا يوجد خامات في مستند التخطيط: {0}» | The issue points at a planning document whose materials grid is empty, so there is nothing to draw from the store. | Open the planning document and run its optimisation so the materials are worked out, then come back. |
| *Could not find source line for item class {0} - {1} - {2} in planning document {3}* — «لا يمكن ايجاد سطر المصدر لتصنيف الصنف {0} - {1} - {2} في مستند التخطيط {3}» | A line on the issue describes a combination of the two item classes and the box that the planning document never produced — usually because the line was typed by hand, or the planning document was re-optimised after the issue was built. | Take the lines from the planning document rather than typing them, and rebuild the issue if the plan has changed since. |
| *Could not find production order generated by planning document: {0}* — «لا يوجد أمر انتاج منشأ بواسطة مستند التخطيط: {0}» | Committing needs the production order that the planning document generated, and no such order exists — the plan was never turned into orders, or the order was deleted. | Generate the production orders from the planning document first; the material issue charges its material to them. |
