...
**إعداد الجانب المحاسبي (Side Configuration)** `termConfig.config.debit.sideConfig` — يوجّه الجانب إلى سجل [جانب محاسبي](/ar/platform/accounting-side-config): نفس كتلة الإعدادات محفوظة مرة واحدة كملف رئيسي ومُشتركة بين ما شئت من التوجيهات.

::: warning إعداد الجانب المحاسبي يحلّ محلّ الكتلة ولا يندمج معها
بمجرد ملء هذا الحقل يُتجاهل كل حقل آخر في نفس الكتلة — الحساب، ونوع الحافظة، والبيانات، ومصادر المحددات، وكل شيء. وتظل الحقول ظاهرة على الشاشة تعرض ما كُتب فيها من قبل، فيبدو الجانب وكأنه يرحّل إلى حساب بينما هو يرحّل إلى حساب آخر تمامًا. أفرغ حقل إعداد الجانب المحاسبي للعودة إلى القيم المكتوبة في الكتلة.
:::
...
**إختصار القيود (Shorten Ledger)** `termConfig.config.shortenLedger` — تلخيص القيد بدلًا من ترحيل سطر لكل سطر مستند. فالسطور المتماثلة تُدمج في سطر واحد، وحيثما التقى مدين ودائن في نفس المجموعة لا يبقى إلا الفرق بينهما. فالفاتورة ذات العشرة سطور التي تجعل نفس العميل مدينًا عشر مرات تُرحّل سطر عميل واحدًا بالإجمالي.

ولا يُعدّ سطران "متماثلين" إلا إذا تطابق كل شيء فيهما: الحساب، والحساب الفرعي، والبعد الكياني، ومحددات مراكز التكلفة الأربعة، والمراجع العامة الثلاثة، وسطرا البيان، والعملة وسعر التحويل. وأي شيء يختلف من سطر لآخر يبقي السطور منفصلة — وقالب بيان يطبع اسم الصنف هو السبب المعتاد في أن يخرج قيدٌ كان متوقَّعًا أن يُختصر بطوله كاملًا.

ينطبق على الفواتير، مردود المبيعات/المشتريات، التوريد/الصرف/التحويل المخزني، وطلب مردود المبيعات.
...
### الجانب الآخر للضريبة

**الجانب الآخر للضريبة 1..4 (Tax 1..4 other side)** `termConfig.taxesOtherSide.tax1OtherSide` … `termConfig.taxesOtherSide.tax4OtherSide` — كل حقل من هذه الحقول يحمل سجل [جانب محاسبي](/ar/platform/accounting-side-config). فهو مرجع إلى جانب محاسبي محفوظ، لا مفتاحًا ولا كتلة تملؤها هنا، ومن ثَمّ فإن توجيه الطرف المقابل للضريبة يبدأ دائمًا بإنشاء ذلك السجل الرئيسي.

ويجيب الحقل عن سؤال واحد — حين يُرحّل جانب الضريبة، فبمقابل ماذا يُرحّل؟ — وتحكم الإجابةَ قاعدتان.

**لا يوجد طرف مقابل إلا لضريبة مضافة فوق الإجمالي.** فكون الضريبة داخلة في الإجمالي أو خارجه تحدده إعدادات الضريبة خلف خطة الضرائب على المستند، لا هذا الحقل. فإذا كانت الضريبة داخلة في السعر فإن سطر العميل نفسه يحملها بالفعل ولا يُنشأ طرف مقابل أصلًا — وما في هذا الحقل لا يُستعمل حينئذ إطلاقًا. أما إذا كانت مضافة فوق الإجمالي فيظهر الطرف المقابل، ويحدد هذا الحقل أين يذهب.

**وإذا تُرك فارغًا هبط الطرف المقابل على الجانب الرئيسي للمستند نفسه.** ففي فاتورة المبيعات تُجعل الضريبة دائنة في حساب الضريبة، ويُرحّل الطرف المقابل مدينًا على الجانب المدين الرئيسي — أي العميل. وفي فاتورة المشتريات ينعكس الاتجاهان. فالحقل الفارغ لا يعني "بلا تأثير" بل يعني "ضعه على العميل / المورد".

خذ فاتورة مبيعات بسطر واحد قيمته 1,000 وضريبة قيمة مضافة 15% **غير** داخلة في الإجمالي، على توجيه جانبه المدين الرئيسي هو العميل، والدائن الرئيسي هو الإيراد، وجانب الضريبة 1 هو حساب ضريبة القيمة المضافة المستحقة:

| سطر القيد | الحساب | مدين | دائن |
|---|---|---|---|
| المدين الرئيسي | العميل | 1,000 | |
| الدائن الرئيسي | الإيراد | | 1,000 |
| الضريبة 1 | ضريبة قيمة مضافة مستحقة | | 150 |
| الجانب الآخر للضريبة 1 (فارغ ← المدين الرئيسي) | العميل | 150 | |

ومع تفعيل **إختصار القيود** يُدمج سطرا العميل، فيصير القيد: العميل مدين 1,150، والإيراد دائن 1,000، وضريبة القيمة المضافة دائنة 150 — وهو قيد الفاتورة الثلاثي المعتاد الذي يتوقعه الجميع.

::: tip نفس السطور الثلاثة تصل من طريقين مختلفين
لو كانت الضريبة *داخلة* في الإجمالي لكان سطر العميل نفسه 1,150 من البداية ولما أُنشئ أي طرف مقابل — نفس القيد الثلاثي، وقد وصل دون أن يكون لهذا الحقل أي دور. ولهذا قد يبدو توجيه وكأنه خاطئ الإعداد بينما يرحّل صحيحًا، ولهذا أيضًا قد يغيّر تعديلٌ في إعدادات الضريبة قيدًا لم يمسّه أحد.
:::

ووجّه الحقل إلى سجل لحساب وسيط للضرائب مثلًا فلا يتحرك إلا السطر الأخير: يصير العميل مدينًا بـ 1,000 ويحمل الحساب الوسيط الـ 150.
...
**الجوانب الأخرى للخصم (Discount other sides)** `termConfig.taxesOtherSide.discount1OtherSide` … `termConfig.taxesOtherSide.discount8OtherSide`، و`termConfig.taxesOtherSide.headerDiscountOtherSide` — كالجوانب الأخرى للضرائب، يحمل كلٌّ منها سجل [جانب محاسبي](/ar/platform/accounting-side-config). لكن ما يفعله به مختلف: فهو لا يعيد توجيه سطر، بل يضيف سطرين.

الخصم يُرحّل عادةً على جانب واحد فقط. ففي فاتورة مبيعات بسطر قيمته 1,000 وخصم 50، يكون الإيراد دائنًا بـ 1,000، وحساب الخصم مدينًا بـ 50، والعميل — الذي صار سطره صافيًا بعد الخصم — مدينًا بـ 950. (وإن لم يُضبط جانب محاسبي للخصم نفسه فإن الـ 50 المدينة ترتد إلى الجانب الدائن الرئيسي للمستند، أي أن الخصم يُخصم مباشرة من الإيراد.)

واملأ الجانب الآخر للخصم ينضمّ إلى القيد سطران آخران: السجل المذكور يصير دائنًا بـ 50، والجانب الرئيسي للمستند نفسه يصير مدينًا بـ 50. فيحمل العميل حينئذ الـ 1,000 كاملة، ويموّل الحسابُ المذكور الخصمَ بدلًا من أن يخفّض ما على العميل في صمت.
