...
أكثر مجموعة استخدامًا مع عملاء MCP الخارجيين هي **أدوات تصدير واستيراد السجلات**، التي تتيح قراءة بيانات النظام واستيراد سجلات جديدة بصيغة JSON. وهي ثلاثة أصناف تولّد تسع أدوات مجتمعةً، ويضيفها زر **إضافة أدوات التصدير (Add Export Tools)** أعلى الجدول ثلاثتها بضغطة واحدة — يضيف الأسطر الناقصة ويملأ وصف كل أداة تلقائيًا:
...
زر **Add Export Tools** يضيف الأصناف الثلاثة أعلاه فقط، لكن النظام يحوي أدوات جاهزة أخرى تضيفها يدويًا باختيار اسم صنفها من قائمة اقتراحات **Tool Class Name**:
...
| `AITTermAndConfigReadTools` | `<prefix>ListTermAndConfigTargets` و `<prefix>GetTermOrConfigSchema` و `<prefix>ReadTermOrConfig` | سرد التوجيهات وإدخالات الإعدادات القابلة للتهيئة، ووصف إعدادات هدف واحد منها بنفس تجميع شاشته، وقراءة القيم المضبوطة فيه فعلًا — وهي إعدادات لا تصل إليها أدوات السجلات أعلاه |
...
فُصلت القراءة عن الكتابة في أصناف مستقلة عمدًا حتى تستطيع الاستغناء عن أصناف الكتابة: `AITTermAndConfigReadTools` وحده — بدون `AITTermAndConfigWriteTools` — يجعل المساعد يشرح كيف هُيِّئ التوجيه دون أن يملك وسيلة لتغييره، ومثله `AITEntityMetadataTools` و `AITReadRecordTools` بدون `AITImportTools`. وانتبه إلى أن زر **Add Export Tools** يضيف `AITImportTools` مع الصنفين الآخرين، فاحذف سطره بعد الضغط عليه إن كان التعريف للقراءة فقط.
:::
