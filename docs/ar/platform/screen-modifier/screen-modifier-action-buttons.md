...
| **قالب الوصلة** (URL Template) | يفتح **أي** وصلة يبنيها النظام من السجل الحالي — عنوانًا خارجيًّا تدخل فيه قيم السجل، أو وصلة إلى سجل آخر، أو وصلة **تفتح سجلًّا جديدًا مملوءًا مسبقًا**. وتشرحه بقية الصفحة بالتفصيل. |
...
## عمود قالب الوصلة: أي وصلة كانت

عند **قالب الوصلة** يكفّ الجدول عن كونه قائمة بأشياء تُشغَّل، ويصير شيئًا أوسع كثيرًا. وليست هناك قائمة
محصورة بما يجوز أن تضعه فيه. فما تكتبه يُعامَل قالب [تِمبو](/ar/admin/tempo.md)، ويُقيَّم على الخادم في
مقابل السجل الذي يفتحه المستخدم، و**ما يُخرجه القالب — أيًّا كان — يُفتح بوصفه وصلة**: عنوانًا على
الإنترنت، أو صفحة في أحد أنظمتك أنت، أو سجلًّا آخر داخل نما، أو مستندًا جديدًا مملوءًا مسبقًا. والعمود لا
يعلم أيَّها هو ولا يعنيه ذلك؛ إنما يفتح الناتج.

ومن ثَمّ فالناس يبنون به ثلاثة أشياء مختلفة تمامًا.

### ١. عنوان تكتبه بنفسك

أبسط الاستخدامات وأقصرها طريقًا، ولا يحتاج صياغة خاصة أصلًا. اكتب العنوان، وضع أي حقل من السجل بين
قوسين معقوفين حيث تريد قيمته:

```tempo
https://www.google.com/search?q={name2}
```

اضغط ذلك على عميل، فيفتح المتصفح بحثًا عن اسمه الإنجليزي. وهذا الشكل نفسه يغطي معظم ما يريده الناس فعلًا
من هذا العمود: ابحث عن هذا العميل في بوابة مصلحة الضرائب، أو افتح هذه الشحنة في صفحة تتبّع شركة الشحن، أو
ضع هذا العنوان في خريطة، أو سلّم كود هذا السجل لتطبيق ويب تشغّله شركتك بجوار نما:
...
ولأن القالب يُقيَّم في وضع السجل، تعمل المسارات المنقوطة مثل `customerClass.code`، فتستطيع أن تمدّ يدك
عبر حقل مرجع إلى حقل في السجل الذي يشير إليه.

والوصلة التي تبدأ بـ `http://` أو `https://` تُعامَل موقعًا خارجيًّا وتُفتح في **تبويب متصفح جديد** من
تلقاء نفسها، ولا تحتاج أن تطلب ذلك. وأي شيء آخر يُفهم موضعًا **داخل نما** ويحلّ محلّ الصفحة الحالية؛ ضع
`{openinnewwindow}` في أول القالب لتفتح وصلة داخلية في تبويب جديد بدلًا من ذلك.

### ٢. وصلة إلى سجل أو قائمة أخرى داخل نما

في تِمبو عناصر تبني لك وصلات داخل النظام: `{link(...)}` إلى سجل قائم، و`{reportlink(...)}` إلى تقرير،
و`{flow(...)}` إلى مسار كيان. وهي تعمل هنا، مع مأخذ واحد يستحق أن يُقرأ مرتين.

::: danger هذه العناصر تحتاج `plainlink=true`، وعلامات التنصيص فارقة
عنصر الوصلة يُخرج افتراضيًّا وسم HTML كاملًا — `<a href='…'>سلاسل التجزئة</a>` — لأنه صُمّم لمتن رسالة
تنويه. وفي هذا العمود يصير النصُّ المُخرَج كلُّه هو العنوان، فيُنتج الوسمُ عنوانًا مشوّهًا و**خطأ 404**.
أضف `plainlink=true` لتحصل على العنوان المجرّد:

```tempo
{link(customerClass,plainlink=true)}
```

واكتبها **بلا علامات تنصيص**. فـ `plainlink="true"` يتجاهلها `{link}` و`{titledlink}` و`{flow}` بصمت،
فتحصل على الـ 404 دون أن تعرف السبب. وإن شئت، فـ `{plainlinks}` في أول القالب تؤدي الغرض نفسه لكل
عناصر الوصلات فيه.

و`{creator(...)}` هو الاستثناء — لا يحتاج شيئًا من هذا ويعمل كما تكتبه.
:::

### ٣. سجل جديد مملوء مسبقًا

وهو المُنشئ، وموضوع القسم التالي. وهو أكثر استخدامات هذا العمود شيوعًا بفارق كبير، لكنه واحد من ثلاثة
خيارات، لا الغرض من العمود.

::: warning لا بد أن يكون السجل محفوظًا
أيًّا كان ما تبنيه من الثلاثة، فالزر يسلّم الخادمَ هويةَ السجل، فهو لا يعمل إلا على سجل موجود فعلًا.
اضغطه على شاشة فيها تعديلات غير محفوظة فيردّ نما بـ **«Record Must Be Saved»** ولا يفعل شيئًا آخر.
احفظ أولًا.
:::

## إنشاء سجل مملوء مسبقًا من زر

هذا هو ثالث الاستخدامات الثلاثة السابقة، وجواب السؤال الذي يقود معظم الناس إلى هذه الصفحة: *كيف أضع
زرًّا في شاشة العميل يفتح فاتورة مبيعات جديدة والعميل مكتوب فيها سلفًا؟*
