...
وهذه واحدة منها وهي تعمل:

![لوحة النظرة العامة للمجموعة: شريط الأدوات وفيه زر الفلاتر، وصف من أربعة مقسِّمات (الشركة، الفرع، من 01-01-2026، إلى 31-08-2026)، وشريط مؤشرات المجموعة المكوَّن من ست بطاقات، ومخطط أعمدة للإيراد مقابل التكلفة بجانب مخطط حلقي لحصة الإيراد، ويبدأ أسفلهما المخطط المركب الشهري](images/bi-dashboard-view-mode-ar.png)

تُعرض لوحات المعلومات على **الصفحة الرئيسية** — هناك يلتقي بها المستخدمون يوماً بيوم. ولفتح لوحة بعينها أثناء إعدادك لها، افتح سجل لوحة المعلومات واضغط زر **فتح لوحة المعلومات (Open Dashboard)** في شريط الإجراءات. وبعد ظهور اللوحة، يحمل شريط أدواتها أزرار **التحديث (Refresh)** و**فتح لوحة المعلومات (Open Dashboard)** و**خيارات العرض (View Options)** (زر ⋮) و**الفلاتر (Filters)**.
...
### مخططات الدائرة والحلقة (Pie & Donut Charts) — 7 قوالب
دائرة قياسية، حلقة (donut)، حلقة مع تسمية مركزية، دائرة وردية (rose — حجم الشريحة بالمساحة لا بالزاوية)، دائرة نصف دائرة، ودائرة متداخلة من مستويين، ودائرة متداخلة من مستويين بتسميات غنية.

### مخططات العدادات (Gauge Charts) — 4 قوالب
عداد عقرب قياسي، وعداد تقدّم، وعداد حلقي، وعداد متعدد الحلقات (مؤشرات متحدة المركز).
...
مصمم المخططات هو الحوار الذي تفتحه عند إنشاء أو تعديل إعداد مخطط widget.

وهو **غير** متاح من لوحة المعلومات نفسها. للوصول إليه، افتح سجل الـ widget — إما عبر **Open Widget** في صندوق أدوات المخطط، أو بفتح شاشة Dashboard Widget والبحث عن الـ widget بكوده — ثم اضغط إجراء **Edit Chart Config** في التبويب الرئيسي (Main) للسجل.

وللمصمم حالتان:

### الحالة A — معرض القوالب (Template Gallery)

عند البدء بـ widget فارغ، يفتح المصمم في وضع المعرض. ترى شبكة قابلة للبحث من جميع الـ 41 قالباً، كل منها مُصيَّر كمخطط مصغّر حي لترى تماماً ما ستختار.

![معرض القوالب داخل مصمم المخططات: مربع بحث، وصف من رقائق تصنيفات أنواع المخططات، وشبكة بطاقات معاينة (Bar Chart وMulti-Series Bar وStacked Bar و100% Stacked Bar وHorizontal Bar وHorizontal Stacked Bar) كل منها موسوم بشكل فتحاته مثل 1D + 1M، وزر Start Blank](images/bi-template-gallery-ar.png)
...
بعد اختيار قالب (أو فتح widget موجود)، تنتقل إلى المحرر. هنا تربط بياناتك، وتضبط مظهر المخطط، وتهيّئ التفاعلات. المحرر مقسّم إلى **منطقة النموذج** على جانب و**معاينة حية** على الجانب الآخر — كل تغيير تجريه يحدّث المعاينة فوراً.

![محرر إعداد المخطط: شريط أيقونات أنواع المخططات في الأعلى، وصف التبويبات (Data Mapping وStyle Options وClick & Links وDrill-Down Mapping) مع زر Change Template، ونموذج الربط على جانب ومعاينة حية لمخطط الأعمدة على الجانب الآخر](images/bi-chart-editor-overview-ar.png)
...
منطقة النموذج تحتوي على أربعة تبويبات — **Data Mapping** و**Style Options** و**Click & Links** و**Drill-Down Mapping** — إضافة إلى زر **Change Template** الذي يعيدك إلى المعرض. ويتغيّر التبويب الأول بحسب نوع الـ widget: widgets الـ Enhanced Table تعرض **Table Columns** مكان Data Mapping، وwidgets بطاقات المؤشرات تعرض **Card Template** وليست لها لوحة معاينة أصلاً.

أما widget **كتلة النص (Text Block)** فيتخطى ذلك كله. مصممه لوحة محتوى واحدة: محرر نص غني يعلوه تبويبان **نص عربي** و**نص إنجليزي**، وصندوق HTML خام قابل للطي للصق الوسوم مباشرة، وأسفلهما جدول أنماط الإطار (لون الخلفية والنص، والحشو الداخلي، ولون الحد وعرضه واستدارته، وحجم الخط الافتراضي، والمحاذاة). اكتب الكتلة بلغة واحدة واترك الأخرى فارغة فيخدم النص نفسه اللغتين؛ واملأ الاثنتين فتقرأ كل لغة نصها.

#### تبويب ربط البيانات (Data Mapping Tab)

هنا تخبر المخطط بحقول البيانات التي يستخدمها.

![تبويب Data Mapping: نوع الربط CategoryValue، وعمود التصنيف leName2، وكتلتا سلسلة (revenue باسم Revenue وcost باسم Cost) لكل منهما نوع السلسلة وتنسيقها، وأقسام مطوية لتنسيق الأرقام والخيارات المتقدمة وTop N With Others وEChart Option](images/bi-designer-data-mapping-tab-ar.png)

الحقول المتاحة تعتمد على نوع ربط البيانات (الذي يُضبط تلقائياً عند اختيار القالب، لكن يمكن تغييره):
...
#### تبويب النمط (Style Options)

التخصيص البصري دون لمس JSON.

![تبويب Style Options مع فتح قسم الألوان واللوحات لعرض ستة صفوف من عيّنات الألوان المسمّاة (Default ونما وWarm وCool وPastel وMono) ومفتاح أنماط الزخرفة، وقسم المفسّر (Legend) مفتوحاً بتحكمات الإظهار والموضع والاتجاه](images/bi-designer-style-tab-ar.png)

والتحكمات موزّعة على أقسام قابلة للطي:
...
#### محرر JSON لخيار ECharts (EChart Option)

لا يوجد تبويب JSON منفصل. فلأي شيء لا تغطيه تحكمات واجهة المستخدم، افتح قسم **EChart Option** أسفل تبويب **Data Mapping** أو تبويب **Style Options** — كلاهما يحمله — وعدّل ECharts option JSON الخام مباشرة في محرر الكود الذي يظهر.

![قسم EChart Option مفتوحاً على محرر JSON بتلوين صياغي يعرض خيار ECharts الخام، بما فيه العنصر النائب للبيانات ‎$DATA.categories](images/bi-designer-json-editor-ar.png)

التغييرات في محرر JSON تنعكس على تحكمات واجهة المستخدم، لذلك يمكنك الجمع بين التعديل البصري وكتابة JSON يدوياً.
...
![تبويب Click & Links مع فتح مُدخَل إصدار فلتر متقاطع واحد، يعرض كود الفلتر المتقاطع، وربط أعمدة المعرّف والكود والاسم، ونوع الكيان «شركة»، ويعلو قسم Link Mappings الفارغ](images/bi-designer-click-links-tab-ar.png)

#### تبويب Drill-Down Mapping

هيّئ أهداف drill-down — مشروح في قسم [Drill-Down](#Drill-Down) أدناه.

![تبويب Drill-Down Mapping مع فتح هدف واحد: مفتاحه، ومفتاح تبديل Widget/Dashboard مضبوط على Widget، وكود الـ widget المستهدف، وحقلا العنوان العربي والإنجليزي، وترتيبه في القائمة، وقائمة أعمدة الفلترة](images/bi-designer-drilldown-tab-ar.png)
...
والجزء الذي يفاجئ الناس هو معنى هذه الصفوف والأعمدة فعلاً: إنها **نِسَب لا خلايا ثابتة**. اللوحة المضبوطة على 12 صفاً تأخذ ارتفاعها على الشاشة وتقسمه إلى اثني عشر، فيأخذ الـ widget الذي ارتفاعه 4 صفوف ثلث الشاشة. واضبط نفس اللوحة على 24 صفاً يصبح نفس الـ widget سُدس الشاشة — لم يُضَف شيء، بل صغُر كل شيء. لذلك حين يبدو الجدول أقصر مما ينبغي، فالحل أن تزيد ارتفاع *ذلك الـ widget* نفسه، لا أن ترفع عدد صفوف اللوحة.

والتمرير ينتج عن الحساب نفسه. ارتفاع اللوحة شاشة واحدة لكل «عدد صفوف» من الصفوف، فهي لا تمرّر إلا حين يتجاوز أدنى widget عدد الصفوف. أو بعبارة أخرى: إذا كان الـ widget السفلي ينتهي على آخر صف، فاللوحة تَعِد بأن تسع شاشة واحدة، وستفي بوعدها بضغط كل شيء — والجداول أولاً — حتى تسعها. ولتحصل على تقرير قابل للتمرير، امنح الـ widgets الارتفاعات التي تستحقها ودعها تتجاوز الأسفل.

ويعالج [المرجع التقني](./bi-module-technical-reference.md#tHdyd-lHjm-wltmryr-Sizing-and-scrolling) هذا بالتفصيل: المعادلات، ومقارنة عملية، وقائمة أنواع الـ widgets التي تحدد ارتفاعها من محتواها وتتجاهل الارتفاع الذي تمنحه لها.
...
يُفتح محرر التخطيط من سجل لوحة المعلومات: اضغط زر **تعديل عناصر لوحة المعلومات (Edit Dashboard Elements)** في شريط الإجراءات. عندها يُحاط كل widget بإطار متقطع يحمل اسمه، مع زر إزالة ومقابض سحب على كل حافة، ويظهر زر **Add Widget** فوق الشبكة.

![محرر التخطيط: زر Add Widget في الأعلى، وكل widget محاط بإطار متقطع يحمل اسمه وزر إزالة ومقابض سحب على كل حافة، مع زرَّي OK وCancel في الأسفل](images/bi-layout-edit-mode-ar.png)
...
أما عددا الصفوف والأعمدة نفسيهما فلا يُعدَّلان في المصمم — بل هما حقلا **عدد الصفوف (Rows Count)** و**عدد الأعمدة (Columns Count)** على سجل لوحة المعلومات. غيّر أحدهما فيتغير حجم كل ما على اللوحة دفعة واحدة، لأنك غيّرت المقياس الذي تُقاس عليه ارتفاعات الـ widgets لا حجم خلية. وهناك حقلان مجاوران على نفس الشاشة يسهل الخلط بينهما وبين هذين: **عدد جميع الصفوف في اللوحة (Total Dashboard Rows Count)** بقيّة يحتفظ بها محرر التخطيط لنفسه ولا أثر لها على طريقة عرض اللوحة، و**أقصى عدد صفوف للموبايل (Mobile Max Rows Count)** لا يُطبَّق إلا على الهواتف، حيث تُلغى الشبكة تماماً وتُرصّ الـ widgets بعرض كامل — وهذا الحقل يحدد كم منها يظهر على شاشة ضيقة واحدة.

وكل تبويب في لوحة Tabbed يقسم ارتفاعه هو على عدد صفوفه هو، فتُضبط التبويبات واحداً واحداً ولا يؤثر تغيير أحدها في غيره.
...
| **Metrics Cards v2 (Enhanced)** | شريط بطاقات مؤشرات أغنى — البطاقات والخانات وخطوط الاتجاه المصغّرة معرَّفة في JSON. انظر [مرجع بطاقات المؤشرات المحسّنة](./bi-reference-enhanced-metrics-card.md) |
...
![شريط التبويبات السبعة للوحة التنفيذية وتبويب Overview محدد، يشارك صفَّه أزرار التحديث وفتح لوحة المعلومات وخيارات العرض والفلاتر، وتحته المقسِّمات وبطاقات المؤشرات](images/bi-tabbed-dashboard-tabs-ar.png)
...
![سجل لوحة المعلومات ونوعها مضبوط على Tabbed، وشريط الإجراءات (تعديل عناصر لوحة المعلومات / فتح لوحة المعلومات / تصدير JSON للوحة)، وجدول اللوحات الفرعية يسرد لوحاتها السبع بعناوين تبويباتها العربية والإنجليزية](images/bi-tabbed-sub-dashboards-grid-ar.png)
...
شريط الفلتر المتقاطع في أعلى لوحة Tabbed هو **اتحاد** كل الفلاتر المتقاطعة المعرّفة على أي من اللوحات الفرعية، مع إزالة التكرارات. لذلك إذا ربطت كل من لوحة المستودع ولوحة المبيعات فلتر "الفرع"، ترى رقيقة فرع واحدة في الشريط — لا اثنتين.
...
وقيم الفلاتر المتقاطعة **عامة، لا خاصة بلوحة واحدة**. فالفلتر الذي تضبطه على لوحة يبقى مضبوطاً عند انتقالك إلى لوحة أخرى — فإن كانت تلك اللوحة مرتبطة بنفس الفلتر المتقاطع، فُتحت وهي مفلترة سلفاً. أزل الرقيقة من شريط الفلاتر متى أردت البدء من صفحة نظيفة.
...
وتعريف الفلتر نفسه يبدو هكذا:

![سجل فلتر متقاطع للشركة: نوع المُدخل حقل مرجع، وقائمة اختيار متعدد تُعرض كقائمة منسدلة، ونوع الكيان المُشار إليه، والعنوانان العربي والإنجليزي، والطرف الأيسر في SQL ومشغّل «Is One Of»](images/bi-crossfilter-entity-screen-ar.png)
...
![شريط أدوات لوحة المعلومات وزر الفلاتر يحمل شارة عدد قيمتها 2، تليه رقيقتا فلتر قابلتان للإزالة مكتوب عليهما Nile Contracting وAlexandria](images/bi-cross-filter-bar-ar.png)

وهذا هو الحوار خلف ذلك الزر:

![حوار الفلاتر المتقاطعة بصفوف للشركة تحمل رقيقة Nile Contracting، وللفرع، ولحقلَي التاريخ «من» و«إلى»، مع زرَّي إلغاء وتطبيق](images/bi-cross-filter-editor-dialog-ar.png)

::: info قد يفتح الحوار فارغاً
إذا كانت لوحة المعلومات تعرض فلاترها المتقاطعة أصلاً كـ widgets مقسِّمة على الشبكة — صف المنتقيات الذي تراه غالباً تحت شريط الأدوات — فتلك الفلاتر تصل مُعلَّمة كمخفية عن الحوار، فيفتح الحوار بلا صفوف على الإطلاق. لا شيء معطّل: اضبط القيم من المقسِّمات نفسها.
:::
...
| نطاق تاريخ (Date range) | مدة كاملة في أداة واحدة | منتقي نطاق بمدد جاهزة — انظر «فلاتر نطاق التاريخ» أدناه |

### فلاتر نطاق التاريخ (Date-Range Filters)

حصر اللوحة بين «أول الشهر واليوم» عبر فلترَي تاريخ منفصلين يعني سجلَّين تحافظ عليهما، وشريحتين في شريط الفلاتر، وفرصتين لأن يفترق طرفا المدة عن بعضهما. وضع علامة **عرض كنطاق تاريخ (Show as Date Range)** على الفلتر المتقاطع يدمج هذين الفلترين في أداة واحدة تحمل طرفَي المدة معاً.

الإعداد يتم على سجل الفلتر المتقاطع: ضع علامة **عرض كنطاق تاريخ** — وهو بجوار نوع المُدخل (Parameter Type) ونوع عرض القائمة (List Display Type) — واجعل **عمود SQL الأيسر (SQL Left Hand Side)** يشير إلى عمود التاريخ المراد حصره. لا حاجة لسجل ثانٍ؛ فالطرفان يخرجان من هذا السجل وحده.

على اللوحة يظهر الفلتر كزر يعرض المدة المختارة. افتحه وستختار إما:

- **مدة جاهزة**، معروضة في ثلاث مجموعات — *الحالية* (اليوم، هذا الأسبوع، هذا الشهر، هذا الربع، هذه السنة)، و*السابقة* (أمس، الأسبوع السابق، الشهر السابق، الربع السابق، السنة السابقة)، و*المتحركة* (آخر 7، آخر 30، آخر 90، آخر 365 يوماً).
- **نطاقاً مخصصاً** — انقر طرفَي المدة على التقويم، أو اكتبهما في خانتَي «من» و«إلى». وإن أدخلتهما بالعكس يُبدَّل ترتيبهما تلقائياً.

في الحالتين يستقبل كل widget مرتبط بهذا الفلتر المتقاطع **شرطين** بدل شرط واحد: يقارَن العمود بـ `>=` بداية المدة وبـ `<=` نهايتها. فلتر واحد، أداة واحدة، وشرطان في SQL.

::: tip المدة الجاهزة تتحرك مع الزمن
اختيار مدة جاهزة يخزّن المدة نفسها، لا التاريخين اللذين تصادف أن حُسبا اليوم. فـ«هذا الشهر» يبقى *هذا* الشهر — افتح اللوحة نفسها الشهر القادم وستغطي الـ widgets الشهر الجديد. أما النطاق المخصص فعلى العكس: يثبّت التاريخين الحرفيين اللذين اخترتهما.
:::

أمران يستحقان الانتباه أثناء الإعداد:

- **المشغّل (Operator)** الخاص بالفلتر لا يُستخدم هنا — فزوج `>=` / `<=` ثابت.
- **الشرط المخصص (Custom WHERE Clause)**، سواء على الفلتر المتقاطع أو على ربط الـ widget، يظل هو الأسبق: يستبدل الزوج المولَّد بـ SQL الذي كتبته، تماماً كما يفعل مع أي فلتر عادي.
...
### الفلاتر المحلية للـ widget (Widget-Local Filters)

أحياناً يحتاج مخطط واحد إلى فلتر خاص به لا تشعر به بقية اللوحة — «أرِني *هذا* المؤشر للمنطقة الشمالية» بينما تبقى الـ widgets المجاورة على إجمالي المجموعة. لهذا وُجدت علامة **محلي للعنصر (Widget-Local Scope)** على سطر ربط الفلتر المتقاطع.

ضع العلامة على سطر الربط — في جدول ربط الفلاتر داخل الـ widget، أو على سطر التجاوز على مستوى اللوحة — فينتقل الفلتر من شريط فلاتر اللوحة إلى الـ widget نفسه: يظهر في الركن العلوي من الـ widget زر فلترة عليه شارة بعدد فلاتره المحلية النشطة، ونقره يفتح حواراً صغيراً فيه مُدخل لكل ربط محلي، مع زر **تطبيق** وزر **مسح الكل**.

والفلاتر المحلية معزولة عمداً عن حوار اللوحة:

- القيمة المنقورة على مخطط مجاور، أو المكتوبة في شريط فلاتر اللوحة، لا تصل إلى ربط محلي — لا يقوده إلا حوار الـ widget نفسه.
- إذا كانت كل ارتباطات فلتر متقاطع معيّن على اللوحة كلها محلية، اختفى ذلك الفلتر من شريط فلاتر اللوحة تماماً.
- والاستثناء هو Drill-Down لأنه إجراء صريح من المستخدم: فالـ drill-down الذي يستهدف فلتراً مرتبطاً محلياً يمرّر قيمته رغم ذلك.

القيم المحلية تُنقل في رابط اللوحة إلى جانب القيم العامة، فالرابط الذي تنسخه يُعيد فتح اللوحة بالحالة نفسها لكل widget.
...
كما يُحاط الـ widget الذي أصدر الفلتر بإطار أعرض من إطار الـ widgets التي تتفاعل معه فقط:

![مخططان متجاوران: الذي أصدر الفلتر محاط بإطار أزرق عريض وعليه شارتان والعمود المنقور مميَّز، بينما المستقبِل للفلتر فقط له إطار رفيع وشارة واحدة](images/bi-widget-filter-badges-ar.png)
...
إذا كانت widget تستخدم مصدر بيانات wizard مع حقول محددات متعددة، يمكنك التعمق *حسب المحدد*. انقر بزر الأيمن "الرياض" ← **Drill down by…** ← الشهر ← ترى نفس المقياس مصنّفاً حسب الشهر للرياض فقط. مسار التنقل (breadcrumb trail) يتتبع مسارك (المنطقة ← الفرع ← الشهر)، ويمكنك النقر على أي خطوة للعودة.

ولا يظهر **Drill down by…** إلا للـ widgets المدعومة بـ wizard. أما الـ widget الذي تأتي بياناته من استعلام SQL خام فلا يعرضه أبداً، مهما بلغ عدد الأعمدة التي يُرجعها الاستعلام، لأن النظام يحتاج هيكل حقول الـ wizard لإعادة بناء الاستعلام أثناء التشغيل.

![نفس المخطط بعد التعمّق حسب الشركة، وقد صار لكل شركة عمود، ومسار التنقل يعرض «Cairo ← Legal Entity»](images/bi-drill-by-breadcrumb-ar.png)
...
![نافذة التعمّق تعرض مخطط صافي المبيعات حسب الشهر مفلتراً على فرع واحد، وفوقه مسار التنقل «Branch: Cairo»](images/bi-drilldown-dialog-ar.png)
...
عند النقر بزر الأيمن على نقطة بيانات في مخطط مُهيَّأ بتفاعلات، ترى قائمة منظمة يعلوها اسم النقطة التي نقرتها، ثم تُجمّع تحتها كل ما يمكنك فعله بتلك النقطة.

![قائمة زر الفأرة الأيمن مفتوحة فوق عمود القاهرة في مخطط، يعلوها اسم النقطة المنقورة، ثم قسم «Open in…» يسرد هدفَي تعمّق، وقسم «Drill down by…» يسرد المحددات غير المستخدمة: الشركة والعميل](images/bi-context-menu-ar.png)

والأقسام هي:

- **التنقل إلى (Navigate To)** — روابط الكيانات وروابط URL المعرّفة على المخطط
- **Open in…** — أهداف التعمّق (widgets أو لوحات معلومات) المهيّأة على المخطط
- **Drill down by…** — المحددات التي يمكنك إعادة تجميع نفس المخطط بها

كل قسم يظهر فقط إذا كان الربط المقابل محدداً، ويشترط في **Drill down by…** زيادةً على ذلك أن يكون الـ widget مدعوماً بمصدر بيانات wizard. ومخطط بلا روابط ولا أهداف drill-down ولا محددات wizard لا يُظهر قائمة سياق على الإطلاق.

---

## شريط أدوات المخطط (The Chart Toolbox)

يستطيع كل مخطط أن يحمل صفاً صغيراً من الأيقونات في ركنه العلوي، لكنها **مخفية افتراضياً**. تُظهرها من شريط أدوات اللوحة: **خيارات العرض (View Options)** — زر الـ ⋮ — ثم **إظهار شريط الأدوات (Show Toolbox)**. والاختيار محفوظ في متصفحك وينطبق على كل لوحة تفتحها.

ومع تشغيل شريط الأدوات يوفّر المخطط:

- **طيّ الـ widget** — يطوي الـ widget إلى شريط عنوان، ونقر الشريط يعيده. والـ widgets المطوية محفوظة لكل لوحة على حدة، فتُفتح اللوحة كما تركتها.
- **حفظ كصورة** — يُنزّل المخطط كملف PNG.
- **عرض البيانات (Data View)** — يعرض الصفوف التي خلف المخطط كجدول للقراءة فقط، وهو أسرع طريق لقراءة الأرقام الدقيقة من صورة.
- **خط / أعمدة / تكديس** — في المخططات المرسومة على محورَي س وص، يبدّل طريقة الرسم في الحال، ومعه **استعادة (Restore)** لإرجاعها. ولا يظهر التكديس إلا إذا كان للمخطط أكثر من سلسلة.
- **اختيار الخانات وقت التشغيل (Runtime Slot Selection)** — في الـ widgets المبنية على مصدر Wizard، يفتح المنتقي الذي يتيح للقارئ تبديل البُعد أو المقياس المرسوم منه المخطط، لهذه الجلسة فقط. انظر [مرجع وضع Wizard](./bi-reference-wizard-mode.md).

ويرى المسؤولون ثلاثة أزرار إضافية: **فتح الـ Widget** الذي يفتح سجل الـ widget، و**نسخ EChart Option** و**نسخ استجابة الخادم**، وهما يضعان JSON إعداد المخطط ورد الخادم الخام في الحافظة — وهما ما تحتاجه بين يديك حين يُرسم مخطط على غير ما توقعت.

أما الـ widgets غير البيانية — الجداول والبطاقات — فتحصل على زر طيّ في الركن نفسه ما دام شريط الأدوات ظاهراً. وwidgets المقسِّمات (Slicers) لا تُطوى.
...
| **شريط الأدوات: عرض البيانات (Data View)** | يفتح صفوف المخطط كجدول للقراءة فقط |
| **شريط الأدوات: نسخ EChart Option** | ينسخ JSON إعداد المخطط إلى الحافظة (للمسؤولين) |
...
محرر إعداد المخطط للـ Enhanced Table widgets يستبدل تبويبة Data Mapping الخاصة بـ ECharts بتبويبة **Table Columns**.

![تبويب Table Columns: مفتاح استخدام تخطيط Pivot، وزرَّا توليد الأعمدة من مجموعة النتائج وإضافة عمود، وستة صفوف أعمدة، وصف واحد مفتوح يعرض معرّف العمود وحقل SQL والرأسين العربي والإنجليزي ومصيّر الشريط وتنسيق النسبة المئوية وعلامتَي التثبيت والإخفاء](images/bi-enhanced-table-columns-tab-ar.png)

ومن تلك التبويبة يمكنك:
...
![سجل معالج widget لوحة المعلومات لمصدر بيانات المبيعات: الجدول الرئيسي SalesInvoiceLine، واستعلام SQL المولَّد، وجدول الحقول تعرض سطوره معرّف كل حقل واستخدامه محدداً أو قياساً وتجميع Sum على القياس](images/bi-wizard-data-source-screen-ar.png)
...
2. **ربطه بفلاتر التاريخ المتقاطعة** — على كل فلتر تاريخ متقاطع (مثل: من تاريخ، إلى تاريخ)، عيّن حقل `Period Comparison Config` للـ config الذي أنشأته. يجب أن يشير كلا فلترَي التاريخ إلى نفس الـ config لإزاحة كلا طرفي النطاق معاً. وإن استخدمت فلتر نطاق تاريخ واحداً بدل زوج «من/إلى»، فعيّن الـ config على ذلك السجل وحده — ويُزاح طرفا مدته معاً.
...
![سجل إعداد مقارنة الفترات: فترة مطروحة مقدارها أربعة أشهر، وأسماء أعمدة الفترة المحقونة، وتسميات الفترات الأربع، وقائمة الفلاتر المتقاطعة التي تستخدمه](images/bi-period-comparison-config-ar.png)
...
![مخطط أعمدة لصافي المبيعات حسب الفرع بعمودين لكل فرع، الفترة الحالية والفترة السابقة، بلونين مختلفين](images/bi-period-comparison-chart-ar.png)
...
![سجل الـ widget مع تفعيل خانة المقارنة، ومفتاح دمج، وجدول Comparison Data Mapping يقرن عمود الإيراد الأصلي بنظيره للفترة السابقة](images/bi-comparison-data-mapping-grid-ar.png)
