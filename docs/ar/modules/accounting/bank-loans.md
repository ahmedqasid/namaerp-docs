...
menu: البنوك ← قروض بنكية ← قرض بنكي
---
...
![شاشة القرض البنكي](images/loans/bank-loan-ar.png)
...
![شاشة سند إصدار القرض](images/loans/loan-issue-ar.png)
...
![شاشة سداد أقساط القرض](images/loans/loan-installment-payment-ar.png)
...
يكاد لا شيء في نظام القروض يُكتب سطرًا سطرًا — فالجداول تُولَّد، ومستندات السداد تجلب المستحق بنفسها.

**في القرض وطلبه ومستند الجدولة:**

- **إنشاء جدول الأقساط** (Generate Installments Table) — يبني جدول الأقساط من قيمة القرض وعدد الأقساط وقيمتها و**بدء الأقساط في** و**يحسب كل**. شغّله كلما غيّرت أيًّا من هذه الأرقام، فيعيد بناء الجدول ليطابقها.
- **إنشاء جدول الفوائد** (Generate Interests Table) — الأمر نفسه لجدول الفوائد، انطلاقًا من تاريخَي البداية والنهاية و**بدء الفوائد في** ودورية الفائدة والنسبة السنوية، ووفق نوع فائدة القرض.
- **إنشاء جدول الأقساط و جدول الفوائد** (Generate Installments And Interests Table) — الجدولان بضغطة واحدة، وهي الطريقة المعتادة لتجهيز قرض جديد.

**وفي طلب القرض** يوجد كذلك:

- **إنشاء القرض** (Generate Bank Loan) — ينشئ القرض نفسه من الطلب المعتمد وينقل شروطه إليه، فلا تعيد إدخالها. يجب حفظ الطلب أولًا، ويُفتح القرض كسجل جديد لتراجعه وتحفظه.

**في سند سداد أقساط القرض:**

- **تجميع الإقساط** (Collect Installments) — تحدّد نطاق **من قرض** / **إلى قرض** (أو بنكًا وحسابًا بنكيًا) ومدى تاريخي، فيملأ المستند بكل قسط **غير مسدَّد** يقع في ذلك المدى، عبر كل القروض المطابقة. وبهذا يسدّد مستند واحد عدة أقساط — أو عدة قروض — دفعة واحدة.
- **تجميع الفوائد** (Generate Interests) — نظيره في الفوائد: يجلب سطور الفوائد المستحقة في المدى نفسه.

**في احتساب فوائد القروض:**

- **تجميع الفوائد** (Collect Interests) — يجمع الفوائد التي لم تُثبَت بعد لنطاق القروض والمدى التاريخي المختارَين في جدول الفوائد. وحقل **من قرض** مطلوب؛ فبدونه ينبّهك الزر ولا يفعل شيئًا.
...
|---|---|---|
| «القسط {0} اكبر من قيمة القسط {1} فى القرض {2}» | مجموع ما سُدِّد على كود قسط واحد — هذا المستند مع كل مستندات **سداد أقساط القرض** المعتمدة الأخرى — صار أكبر من قيمة القسط في القرض. | أنقِص المبلغ المسدَّد في هذا المستند، أو تحقّق من أنّ القسط لم يُسدَّد في مستند آخر. |
| «الفائدة التى تاريخها {0} وقيمتها {1} اكبر من قيمة الفائدة {2} فى القرض {3}» | الفحص نفسه لتجاوز السداد على جانب الفوائد: المسدَّد من الفائدة لذلك التاريخ صار أكبر من سطر الفائدة في القرض. | صحّح الفائدة المسدَّدة، أو أعِد تشغيل **احتساب فوائد القرض** إن كانت الفائدة المستحقة نفسها خاطئة. |
| «لا يمكن ايجاد سطر قسط بكود {0} فى القرض {1}» | سطر دفع يحمل كود قسط لم يبقَ موجودًا في القرض — غالبًا لأنّ **جدولة قرض** أو **مستند تعديل قرض** أعاد بناء الجدول بأكواد مختلفة. | أعِد اختيار القسط في مستند الدفع؛ وإن كان مستند جدولة حذف أقساطًا مسدَّدة فصحّح ذلك المستند. |
| «لا يمكن ايجاد سطر فوائد بتاريخ فائدة {0} وقيمة فائدة {1} فى القرض {2}» | والأمر نفسه لسطر سداد فائدة لم يعد زوجُ (التاريخ، القيمة) فيه مطابقًا لأي سطر فوائد في القرض. | أعِد اختيار سطر الفائدة، أو أعِده إلى القرض. |
