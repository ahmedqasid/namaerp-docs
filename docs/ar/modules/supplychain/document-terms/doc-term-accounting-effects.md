...
## خصم التقريب (Approximation Discount)

قلّما تُسوَّى الفاتورة النقدية حتى آخر قرش. ففاتورة بـ 199.97 يُدفع فيها 200.00، ولا بد للقروش الثلاثة من مستقر — ومستقرها هو جانب **خصم التقريب**. أما هل يحدث التقريب أصلًا، وما أكبر متبقٍّ يُعدّ "قريبًا بما يكفي"، فيتقرر في [الضرائب والفاتورة الإلكترونية](/ar/platform/global-config/global-config-taxes) بالإعدادات العامة؛ ولا يقدّم التوجيه إلا الحساب ومفتاحًا واحدًا للسلوك.

**خصم التقريب (Approximation Discount)** `termConfig.approximationDiscount` — الجانب المحاسبي الذي يستوعب المتبقي الضئيل. فحين يقع المبلغ المتبقي على المستند داخل قيمة التقريب المضبوطة في الإعدادات العامة، يُقيَّد هذا المتبقي هنا بدل أن يظل على حساب العميل (أو المورد)، فتُقفل الفاتورة نظيفة. واتركه فارغًا فلا يحدث تقريب على المستندات التي تستعمل هذا التوجيه مهما كانت الإعدادات العامة.

**اعتبار المدفوعات التي لا تؤثر على المتبقي مع خصم التقريب** `termConfig.considerDoNotAffectRemainingWithApproxDisc` — في توجيهات جانب المبيعات فقط.

يمكن وسم سطر الدفع بـ **عدم التأثير على المتبقي** (سواء على السطر نفسه أو على طريقة الدفع). فمثل هذا الدفع يُسجَّل ويُقيَّد، لكنه عن قصدٍ لا ينقص **المتبقي** على المستند — وهذا هو مقصد الوسم كله، وبه تُعالَج المدفوعات المؤجلة والمحصَّلة عبر طرف ثالث وما شابهها. ونتيجته الجانبية أن رقم *المتبقي* لم يعد يعبّر عمّا على العميل فعلًا.

ولهذا هنا أثر، لأن قرار التقريب يقوم على مقارنة المبلغ المتبقي بقيمة التقريب. خذ فاتورة بـ 200.03 سُدِّدت بدفعة 200.00 على طريقة موسومة بـ *عدم التأثير على المتبقي*: حساب العميل مُسوّى إلا من ثلاثة قروش، ومع ذلك يظل *المتبقي* 200.03. وأمام قيمة تقريب 0.05 فهذا أبعد ما يكون عن "قريب بما يكفي"، فلا يحدث تقريب وتبقى القروش الثلاثة على حساب العميل إلى الأبد.

فإذا أشّرت على هذا الخيار استُبعدت تلك المدفوعات من المقارنة أولًا، فلا يبقى إلا 0.03 المتبقية حقًّا — وهي تقع داخل قيمة التقريب، فتُقيَّد على حساب خصم التقريب وتُقفل الفاتورة. واتركه دون تأشير يظل التوجيه على سلوكه السابق.

::: tip العلاقة بالدفع بالقسائم
الإعداد العام **اعتبار الدفع بالقسائم مع خصم التقريب** يقرّب في مقابل جانب الدفع بالقسائم من القيد. فحين يكون خيار التوجيه هذا مفعّلًا *و* يحمل المستند فعلًا مدفوعات لا تؤثر على المتبقي، يسلك التقريب طريق العميل/المورد بدلًا من ذلك، لأنه الجانب الذي يستقر عليه المتبقي حقًّا. أما على المستندات الخالية من مثل هذه المدفوعات فلا يتغير شيء ويسري الإعداد العام كالمعتاد.
:::
