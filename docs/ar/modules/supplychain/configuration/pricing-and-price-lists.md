...
**عدم تحديث السطور من رأس ملف قوائم الأسعار** (`value.doNotUpdateLinesFromPriceListHeader`) — افتراضياً، كل حفظ لقائمة الأسعار يُنزل بالرأس إلى سطوره: فحيثما حمل الرأس قيمة تُكتب فوق ما كان في السطر — تاريخا من وإلى، والمحددات، والمورد، وتصنيف الفاتورة، ومصنِّفات الأسعار من ١ إلى ٥. وعند التفعيل يتوقف هذا النسخ الشامل، لكنه لا يتوقف كلياً: فالمورد وتصنيف الفاتورة ومصنِّفات الأسعار تُفرض على كل سطر كما هي، والتاريخان يُملآن فقط حيث تركهما السطر فارغين، أما المحددات فلا تُمس.
...
**وضع وصلة تكاليف الصنف في صفحة منفصلة** (`value.addItemCostListToSeparatePage`) *(مفعّل في قواعد البيانات المنشأة حديثاً)* — يتحكّم في مكان ظهور جدول ملخّص تكاليف الصنف على شاشة الصنف: عند التفعيل يُنقَل إلى صفحة "إحصائيات" خاصة به؛ وعند الإطفاء يبقى على صفحة الأسعار.
...
**مجموعة الوحدات / الوحدة الأساسية(الأصغر)** (`value.unitConfig.uomGroup` / `value.unitConfig.baseUnit`) — تحدّد مجموعة وحدة القياس والوحدة الأساسية الافتراضية للأصناف الجديدة. عند إنشاء صنف بدون وحداته الخاصة، يطبّق النظام هذه الوحدة الأساسية والمجموعة على الوحدة الرئيسية للصنف، ويملأ منها وحدات الشراء والبيع والتقارير الافتراضية. اضبطها عندما يشترك معظم الأصناف في نفس الوحدة الأساسية (مثل "قطعة") حتى تأتي الأصناف الجديدة مُعبّأة مسبقاً.
