...
**في مستند بداية الجرد:** **اضافة سند نهاية جرد** (Create End Stock Taking) — يجب حفظ مستند البداية؛ ينشئ مستند النهاية المقابل مشيرًا إلى هذه البداية، فلا يقع خلط بين الزوجين.

**في لجنة الجرد المخزني:**

- **تجميع الأصناف** (Collect Stock Items) — يملأ سطور العد من معايير تجميع الأصناف التي أدخلتها. ويرفض العمل إن كانت تلك السطور فارغة.
- **تجميع الأصناف** (Collect Items) و**تجميع الأصناف في تاريخ** (Collect Items In Period) — يملآن السطور مما يحويه المخزن والموقع فعلًا. والثاني يسأل ثلاثة أسئلة أولًا: التاريخ الذي يجمع عليه، وهل يُدخل الكميات السالبة، وهل يقصر النتيجة على الأصناف الموجودة في التفاصيل.
- **إنشاء سطور لجنة الجرد المخزنى** (Create StockTaking Details Lines) — يبني سطور العد من سطور الجرد الإلكتروني في المستند. ويرفض العمل إن لم تكن هناك سطور، ويطلب منك إدخال مستندات الجرد الإلكتروني أولًا.

**في الجرد الإلكتروني:** **تحويل إلى لجنة جرد** (Convert To StockTaking Details) — ينشئ مستند لجنة جرد من العد الإلكتروني ويفتحه.

**في مستند نهاية الجرد:** **حساب الكميات طبقا للتاريخ** (Preview Stock Items On Date) — يجب حفظ المستند؛ يعرض ما يراه النظام رصيدًا في التاريخ الفعلي لتقارنه قبل الاعتماد. ويحتاج اختيار **مستند بداية جرد**، ولا يعمل إلا إذا كانت طريقة حساب الكميات هي التي تعمل مع التاريخ؛ وإلا أخبرك بذلك ولم يفعل شيئًا.
...
|---|---|---|
| «يوجد بالفعل سند بدء جرد مخزني بتاريخ {0} للمخزن {1} والموقع {2}» | يوجد **سند بدء جرد** محفوظ آخر يفتح جردًا للمخزن نفسه — والموقع نفسه إن حُدِّد — في ذلك التاريخ. | استخدم الجرد المفتوح بالفعل، أو فعّل **السماح بأكثر من جرد في نفس اليوم لنفس المخزن** في إعدادات المخازن إن كنت تجرد المخزن مرتين في اليوم فعلًا. |
| «يوجد بالفعل سند إنهاء جرد مخزني بتاريخ {0} للمخزن {1} والموقع {2}» | الحماية نفسها في جهة الإغلاق: يوجد **سند إنهاء جرد** لذلك المخزن والموقع في التاريخ نفسه. | أغلق الجرد بالسند الموجود، أو اسمح بأكثر من جرد في اليوم من الإعدادات. |
| «رقم المسلسل {0} يجب ان يكون في المخزن {1}, ولكنه موجود في المخزن {2}» | رقم مسلسل مجرود مسجَّل في النظام في مخزن غير المخزن المذكور في سطر الجرد. والرسالة تذكر المخزنين. | إما أن المسلسل انتقل فعليًا بدون مستند — فسجّل التحويل — أو جُرد في المخزن الخطأ. صحّح الخطأ منهما قبل إنهاء الجرد. |
| «لا يمكن تعديل الجرد المخزني إذا كان منتهيا» | يُعدَّل مستند **تفاصيل الجرد** بعد إنهاء الجرد وتسويته، وخيار **السماح بتعديل مستندات الجرد المنتهية** غير مفعَّل في إعدادات المخازن. | اترك الجرد المنتهي وصحّح المخزون بمستند مستقل، أو فعّل ذلك الخيار إن كان لا بد من إعادة فتح الجرد. |
| «المستند {0} مرتبط بالفعل بالمستند {1}» | ورقة جرد إلكتروني مدرجة في مستند التفاصيل مرتبطة بالفعل بمستند تفاصيل آخر. | احذف ذلك السطر، أو افصل الورقة عن المستند الذي تذكره الرسالة. |
| «لا يمكنك ترك لجنة الجرد الإلكتروني، لجنة جرد نقط بيع ولجنة جرد مبدئي فارغين» | سطر في جدول الجرد الإلكتروني بمستند التفاصيل لا يذكر أيًّا من المصادر الثلاثة التي يأتي منها الجرد — جرد إلكتروني أو جرد نقاط بيع أو جرد مبدئي. | املأ أحد الثلاثة في ذلك السطر، أو احذف السطر. |
