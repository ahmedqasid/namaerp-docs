...
menu: الرواتب ← حضور / إنصراف ← إعدادات ماكينة الحضور
---
...
|---|---|
| نوع اتصال الماكينة (Machine Connection Type) | **ZkBioTime** أو **SQLSERVER** أو **ACCESS** أو **Timetaag**. اختيار أحدها يحدد أي التبويبات الأربع أدناه ستملؤها. |
...
![إعدادات ماكينة الحضور، موضحة تبويبات نوع الاتصال](../images/attendance/attendance-machine-config-ar.png)

### أنواع الاتصال الأربعة

لكل نوع تبويبته الخاصة. املأ التبويبة المطابقة لـ **نوع اتصال الماكينة** وتجاهل التبويبات الثلاث الأخرى.

#### ZkBioTime

للمواقع التي تشغّل منصة المورّد الخاصة ZkBioTime، والتي تنشر حركاتها عبر واجهة ويب. وهو مع Timetaag أحد النوعين اللذين لا يحتاجان وصولاً إلى قاعدة بيانات ولا كتابة أي SQL.
...
للمواقع التي ترفع أجهزة البصمة فيها قراءاتها إلى **Timetaag**، وهي خدمة حضور سحابية. وهذا النوع مختلف في طبيعته عن الأنواع الثلاثة الأخرى: فلا توجد قاعدة بيانات للماكينة ولا برنامج محلي يُقرأ منه. فالأجهزة نفسها ترفع قراءاتها إلى Timetaag، ويكتفي التطبيق بسؤال خوادم Timetaag عمّا تراكم لديها.

| الحقل | القيمة |
|---|---|
| رابط خادم Timetaag (Timetaag Server URL) | عنوان خدمة Timetaag، وهو `https://app.timetaag.com`. |
| مفتاح Timetaag (Timetaag API Key) | المفتاح الصادر من Timetaag. وتظهر هذه الخانة مرتين في التبويبة — انظر أدناه. |

ويأتي المفتاح من **Timetaag لا من نظام نما**. ادخل إلى لوحة تحكم Timetaag بحساب مدير الشركة واستخدم **Generate API token**.

وتظهر خانة المفتاح مرتين لأن Timetaag يتوقع أمرين مع كل طلب: مفتاح API يعرّف شركتك، ورمز توثيق. ويرسل التطبيق الخانة الأولى بوصفها مفتاح الـ API والثانية بوصفها رمز التوثيق. فإن أصدر لك Timetaag قيمة واحدة فضعها في الخانتين، وإن أصدر قيمتين فاحتفظ بهما بهذا الترتيب.

وكما في ZkBioTime، تصل القراءات بصيغة ثابتة، ولذلك فإن SQL Query و Read For Period Query وجدول المطابقة في هذه التبويبة **غير مستخدَمة**.

::: tip يحتاج Timetaag إلى الإنترنت لا إلى الشبكة المحلية
تحتاج الأنواع الثلاثة الأخرى أن يكون التطبيق على الشبكة نفسها التي عليها الماكينة أو قاعدة بياناتها. أما Timetaag فيحتاج العكس: يجب أن يكون الجهاز الذي يعمل عليه التطبيق قادراً على الوصول إلى `app.timetaag.com` عبر الإنترنت. والفرع المغلق على حركة داخلية فقط سيفشل هنا رغم صحة كل الإعدادات.
:::
...
واتصال **Timetaag** هو الاستثناء هنا، لأن خدمة Timetaag تُرشِّح بالتاريخ لا بالوقت: فأياً كانت لحظتا البداية والنهاية اللتان يطلبهما التطبيق، تُعيد الخدمة كل قراءة يقع وقت بصمها في تلك التواريخ. ولذلك تُعيد كل عملية جمع مجدوَلة قراءة اليوم الجاري بأكمله، لا الدقائق التي مضت منذ العملية السابقة فحسب. ولأن النظام يتجاهل القراءات التي يملكها بالفعل، فلا ينتج عن ذلك أي تكرار — وإن كان يعني أن جدول إحصائيات التطبيق يعرض عدد القراءات التي كان اليوم يحويها لحظة تنفيذ العملية، لا عدد القراءات الجديدة. فتكرار الرقم نفسه طوال فترة بعد الظهر أمر طبيعي، وليس دليلاً على تعطل عملية الجمع.

كما يرجع التطبيق بنقطة البداية نصف ساعة إلى الوراء في كل عملية، ونصف الساعة هذا يؤدي دوره في لحظة واحدة بعينها: بُعيد منتصف الليل، حين يُعيد طرحُه بدايةَ الفترة إلى التاريخ السابق. فيُجمَع اليوم السابق مرة أخيرة — وبهذا تصل إلى نما بصمةٌ سُجِّلت قُبيل منتصف الليل ورفعها الجهاز بعد دقائق من بدء اليوم الجديد.
