...
::: tip ملاحظة
- افلتر على `n5` لا على `quantities.data.net`. فالصنف الذي لم يتحرك قط في مخزن **ليس له سطر كميات أصلاً**، ولذلك لا يطابق الشرطُ المكتوب على `quantities.data.net` شيئاً — وهذه بعينها الأصناف التي وُضع الفلتر من أجلها. أما الحقل `n5` فيحمل صفراً لها لأن الاستعلام أعلاه يكتب `coalesce(qty.net,0)` لكل صنف.
- وثمن ذلك هو الحداثة: فقيمة `n5` لا تعدو آخر تشغيل للمهمة. فاختر مدة تشغيل المهمة المجدولة بحيث يبقى الحقل دقيقاً دون أن تُثقل الخادم وقاعدة البيانات.
:::
