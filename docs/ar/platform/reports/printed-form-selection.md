...
الأمر عند هذا الحد. وهذا الجواب يُحفظ في ذاكرة صفحة المتصفح ما بقيت مفتوحة، مرتبطاً بنوع السجل والصفحة
والمطالعة. فإذا أضفت نموذجاً لمستخدم قيل له للتو إنه لا يوجد ما يُطبع، وجب على ذلك المستخدم **تحديث
الصفحة** قبل أن يلتفت النظام إلى النموذج الجديد — ويكفي التحديث العادي، ولا حاجة لتسجيل الخروج. وهذه نقطة تُربك كثيرين أثناء التطبيق:
...
- **الصفحة (Form Page)** — تقييد اختياري بصفحات (تبويبات) شاشة السجل التي يُعرض منها النموذج.
  اتركه فارغاً فيُعرض النموذج من كل الصفحات. واضغط داخل الحقل تظهر لك أسماء صفحات شاشة **النوع
  (Form Entity)**؛ اختر واحدة فلا يُعرض النموذج إلا من تلك الصفحة. والمطابقة لكل قيمة متساهلة: تقبل
  مفتاح الترجمة للصفحة، أو عنوانها العربي، أو عنوانها الإنجليزي، أو ترتيبها على الشاشة بدءاً من ١.
  ولهذا يعمل `2` كما يعمل عنوان الصفحة الظاهر.

::: tip نموذج واحد على أكثر من صفحة — افصل الصفحات بفواصل
يقبل حقل **الصفحة** أكثر من صفحة. اكتبها مفصولة بفواصل — `Main,Attachments`، أو `1,3` بالترتيب —
فيكون النموذج مرشحاً من أي صفحة في القائمة. ويساعدك الحقل هنا أيضاً: بعد أن تكتب الفاصلة يقترح
عليك الصفحات التي لم تُدرج بعد، واختيار إحداها يضيفها بعد الفاصلة بدل أن يستبدل ما كتبته. استخدم
الفاصلة اللاتينية `,`؛ فالفاصلة العربية `،` ليست فاصلاً.
:::
...
- **المحددات** — المحددات الخمسة نفسها، لكن هنا كمطابقة لا كفلتر، وبتساهل أكبر قليلاً: [المحدد المركب](/ar/platform/dimensions-and-composite-dimensions)
...
| لا شيء يُطبع، بينما نجحت عند زميل | محددات المستخدم نفسه (قد يكون النموذج غير مرئي له)، أو **للمستخدم**، أو الجواب المحفوظ أدناه — اطلب منه تحديث الصفحة. |
| نموذج أُنشئ للتو ولا يُعرض | الجواب المحفوظ في الصفحة عن «هل يوجد نموذج؟». حدّث الصفحة. |
...
| يعمل من صفحة ولا يظهر من أخرى | **الصفحة (Form Page)** على النموذج — هل الصفحة الغائبة مدرجة في قائمته المفصولة بفواصل؟ |
