...
عند ربط قائمة فحص بصنف (راجع [فهم أصناف المخزون](./understanding-items.md#thyy-ltSnyaa-wDbT-ljwd))، يفرض النظام اجتياز الفحص قبل إتاحة الصنف.
...
**في طلب تأكيد الجودة:** **إنشاء تاكيد جودة** (Create quality assurance doc) — يجب حفظ الطلب؛ ينشئ منه مستند تأكيد الجودة وينقل إليه مهندس الجودة وتسلسل العملية، ويفتحه.

**في طلب فحص الجودة:** **إنشاء سند فحص جودة** (Create QC Document) — الخطوة نفسها لفحص الجودة، إذ يبني مستند الفحص من سطور الطلب.

**وفي مستند فحص الجودة ومستند تأكيد الجودة كليهما:** **إنشاء طلب تحويل مخزني للكميات المقبولة** (Stock Transfer request For Accepted) و**إنشاء طلب تحويل مخزني للكميات المرفوضه** (Stock Transfer request For Rejected) — وهما ما يحرّك البضاعة بعد صدور الحكم. فالأول يصدر طلب تحويل مخزني للكميات التي قبلتها، عادةً من مخزن الحجر إلى المخزن الصالح للاستخدام؛ والثاني يفعل الشيء نفسه للمرفوض، فيرسله حيث يستقر مخزونك المرفوض. والثاني يحتاج المستند محفوظًا أولًا.
