...
ستجد تخطيط الخامات عبر **التصنيع ← Cartoon ← تخطيط خامات الكرتون**.
...
في التجميع يكمن المال. فتخطيط الطلبيات واحدةً واحدة لا يترك للمُحسِّن ما يعمل عليه، بينما تجميع الطلبيات المتوافقة عبر الطلبيات المرافقة يوفّر عادةً 5-10% من المواد، وهذا أكبر رافعة منفردة في هذه الشاشة. لكن ثمة سقفًا — فكثرة الطلبيات بأحجام شديدة التباين في خطة واحدة تجعل المسألة أصعب لا أغنى، وثلاث إلى خمس طلبيات نقطة توازن عملية.

وخطّط بانتظام بدلًا من ادّخار عمل أسبوع. فالدفعات الأصغر والأكثر تواترًا تحفظ مرونة إقحام طلبية عاجلة، وهو ما لا تحفظه خطة أسبوعية ضخمة واحدة.

وقبل قبول الطلبيات المرافقة، استخدم **مراجعة الكميات المتاحة**. فالالتزام بخطة مجمّعة لا تستطيع توفير موادها أسوأ من تخطيط الطلبيات منفصلة، لأنه يفشل لاحقًا وفي لحظة أسوأ.

وافرض العروض بحساب. فالمُحسِّن أبرع في هذا من الحدس، وينبغي قصر فرض العروض على قيود حقيقية — اشتراط عميل، أو تصريف لفّات جزئية، أو قيد ماكينة — لا على تخمين.

وراقب نوع الحل. فتشغيلة تعود دومًا بـ**ممكن** بدلًا من **أمثل** تخبرك بأنها نفدت من الوقت، وخمس دقائق بحث إضافية توفّر 2% من المواد تستحق الانتظار في الغالب الأعم. وحين يفشل التحسين تمامًا، احفظ المستند مسودةً واقرأ الخطأ بدلًا من إعادة المحاولة فورًا: فهو يسمي الأصناف التي تعذّر توفيرها والقيود التي تعذّر استيفاؤها، وإعادة التشغيل دون إصلاحها تفشل من جديد.
