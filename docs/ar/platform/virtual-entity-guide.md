...
menu: إدارة النظام ← التقارير ← كيان افتراضي
---
...
تتوفر شاشة Virtual Entity ضمن **إدارة النظام ← التقارير**، بجوار معالج التقارير ومصدر البيانات. افتحها كما تفتح أي ملف رئيسي: انقر **جديد**، أكمل ترويسة السجل، وستكون جاهزًا.

تحتوي الترويسة على عدد محدود من الحقول:

- **Code** — معرّف قصير، يتكون من حروف أو أرقام أو شرطة سفلية فقط، ويجب أن يبدأ بحرف. يستخدم النظام هذا الكود لتسمية الـ view الأساسي (`vw_<code>`) وبوصفه اسم نوع الكيان. تعامل معه كقيمة نهائية: لا شيء يمنعك من تعديله بعد الحفظ الأول، لكن لا شيء يعيد التسمية أيضًا — يُنشأ view ثانٍ بالاسم الجديد بينما يبقى الـ view القديم في قاعدة البيانات وتظل التقارير المبنية على الكود القديم تشير إليه. إذا احتجت كودًا مختلفًا فأنشئ كيانًا افتراضيًا جديدًا (راجع [ما القادم لاحقًا](#m-lqdm-lHqan)).
...
- **Materialization** — في الوقت الحالي، `View` وحده هو ما يعمل. لا يزال `Table` معروضًا في القائمة المنسدلة ويمكنك اختياره، لكن الحفظ يُرفض برسالة تطلب منك استخدام `View`؛ وهو محجوز لمرحلة مستقبلية لتجسيد البيانات في جدول فيزيائي قابل للتحديث.
...
::: warning محرر التعيينات يحتاج الواجهة الجديدة
محرر التعيينات جزء من واجهة المستخدم الجديدة. افتح شاشة الكيان الافتراضي من هناك ليفتح الزر النافذة الموصوفة أدناه. أما الضغط عليه من الواجهة القديمة فيعطيك رسالة خطأ تفيد بأن المحرر متاح في الواجهة الجديدة فقط — إذ لا تملك الواجهة القديمة وسيلة لعرضه. وإن تعذّر عليك الانتقال إلى الواجهة الجديدة، فاملأ حقل **Column Mapping** يدويًا؛ صيغته موثقة في قسم «حقل Column Mapping» أدناه.
:::

::: tip حين تريد النتيجة وحدها لا جدولًا كاملًا
الكيان الافتراضي يمنح المعالجات *جدولًا* جديدًا تُبنى عليه التقارير وعناصر لوحات المعلومات. أما إن كان مرادك رقمًا واحدًا أو نصًا واحدًا من استعلام يظهر على الشاشة — رصيد العميل المستحق بجوار اسمه في الفاتورة، أو عمودًا إضافيًا في مطالعة القوائم — فذلك **حقل محسوب** يُعرَّف في [أعدادات الحقول و الشاشات](/ar/platform/fields-and-entities-settings/fields-settings-calculated-fields). الفكرة نفسها لكن أصغر بكثير: تكتب الاستعلام مرة فتظهر النتيجة على السجل دون تقرير في المنتصف.
:::
...
كل ما تبنيه النافذة ينتهي في مكان واحد: حقل **Column Mapping** على شاشة الكيان، وهو يحتفظ بقائمة الخصائص في صورة JSON. في الأحوال العادية لن تنظر إليه أبدًا — فنقر **OK** في النافذة يكتبه نيابة عنك. لكنه يهم في حالتين: أن تكون على الواجهة القديمة ولا تستطيع فتح النافذة أصلًا، أو أن تنشئ كيانات افتراضية عبر ملف استيراد.

القيمة **كائن** JSON يحتوي على قائمة واحدة باسم `properties`، مدخل واحد لكل عمود:
...
- **`columnName`** يجب أن يطابق اسم العمود (أو الاسم البديل) في قائمة SELECT تمامًا — فهو ما يربط الخاصية بعمود الـ view.
- **`fullName`** هو الاسم المنطقي للحقل الذي يعرضه المعالج ويبني منه معرفات الحقول.
- **`fieldType`** واحد من أنواع الحقول القياسية، مكتوبًا حرفيًا: `Text` و`Integer` و`Long` و`Decimal` و`Boolean` و`Date` و`Time` و`DateAndTime` و`Reference` و`BigText` و`Enum`.
- **`referenceTo`** يحدد الكيان الهدف، ولا ينطبق إلا حين يكون النوع `Reference`؛ اتركه `null` فيما عدا ذلك.
- **`arabicName` / `englishName`** هما التسميتان اللتان يراهما المستخدمون في منتقي الحقول وفي عناوين أعمدة التقرير.

::: warning الغلاف ليس اختياريًا
المصفوفة المجردة — `[{"columnName": …}, …]` دون الغلاف `{"properties": …}` المحيط بها — هي الخطأ الأكثر شيوعًا. الحفظ الآن يُرفض برسالة توضح ذلك، لكن في الإصدارات السابقة لهذا التحقق كان يُقبل بصمت وينتج كيانًا افتراضيًا بلا أعمدة صالحة للاستخدام إطلاقًا.
:::
...
::: tip التركيبات التي تعمل على أكثر من خادم تطبيقات
هذا التحديث للذاكرة المؤقتة يصل إلى الخادم الذي نفّذ الحفظ. فإذا كانت التركيبة تعمل على عدة خوادم تطبيقات خلف موزّع أحمال، تظل الخوادم الأخرى تقدّم النموذج الذي لديها، وقد يغيب كيان افتراضي جديد عن منتقي الجداول لمن يصل إلى أحدها. إعادة تشغيل تلك الخوادم تحل المشكلة. وإذا لم تجد كيانًا افتراضيًا حفظته للتو رغم وجود الـ view الخاص به في قاعدة البيانات، فابدأ بالتحقق من هذا.
:::
...
افتح **Virtual Entity** من **إدارة النظام ← التقارير**، انقر **جديد**، وأدخل:
...
| SQL يحتوي على معامل `:placeholder` | لا شيء يوقف الحفظ عند التحقق، لكن SQL Server لا يقبل معاملًا داخل view، فيفشل View DDL ويفشل الحفظ برسالة خطأ منه. يجب أن يكون SQL للكيان الافتراضي مكتفيًا بذاته — اجعل المطالبة بالقيم عبر مدخلات التقرير المستهلك. |
| تغير **Code** بعد الحفظ الأول | مقبول، ونادرًا ما يكون هو المطلوب: يُنشأ view ثانٍ بالاسم الجديد، ويبقى الـ view القديم في قاعدة البيانات، وتظل التقارير المبنية على الكود القديم تشير إليه. أنشئ كيانًا افتراضيًا جديدًا بدلًا من ذلك. |
| JSON في **Column Mapping** غير سليم أو مصفوفة مجردة | مرفوض عند الحفظ برسالة تحدد المشكلة. الإصدارات الأقدم كانت تقبله بصمت وتترك الكيان بلا أعمدة. |
...
في الوقت الحالي، اعتبر Materialization إشارة خارطة طريق: `Table` مدرج في القائمة المنسدلة لكن الحفظ به يُرفض.
