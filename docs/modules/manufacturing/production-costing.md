# تكاليف الإنتاج: تتبع الأموال {#Production-Costing--Following-the-Money}

## التكلفة الحقيقية للتصنيع {#The-Real-Cost-of-Making-Things}
...
## فهم تكاليف التصنيع: الثلاثة دلاء {#Understanding-Manufacturing-Costs--The-Three-Buckets}

تخيل أن تكاليف التصنيع تقع في ثلاثة دلاء. اثنان منها سهلا التتبع، والثالث... معقد.

### الدلو الأول: المواد المباشرة - السهل {#Bucket-1--Direct-Materials---The-Easy-One}
...
### الدلو الثاني: العمالة المباشرة ووقت الآلات - لا يزال واضحاً {#Bucket-2--Direct-Labor-and-Machine-Time---Still-Pretty-Clear}
...
### الدلو الثالث: التكاليف الغير مباشرة - المعقد {#Bucket-3--Manufacturing-Overhead---The-Complicated-One}
...
## توزيع التكاليف الغير مباشرة: إظهار الخفيّ {#Overhead-Allocation--Making-the-Invisible-Visible}
...
### الطريقة 1: مبلغ ثابت لكل أمر {#Method-1--Fixed-Amount-per-Order}
...
### الطريقة 2: نسبة مئوية من تكلفة المواد {#Method-2--Percentage-of-Material-Cost}
...
### الطريقة 3: نسبة مئوية من تكلفة العمالة {#Method-3--Percentage-of-Labor-Cost}
...
### الطريقة 4: سعر لكل ساعة إنتاج {#Method-4--Rate-per-Production-Hour}
...
### الطريقة 5: سعر لكل وحدة منتَجة {#Method-5--Rate-per-Unit-Produced}
...
### الطريقة 6: حساب التكاليف الغير مباشرة الفعلية - الواقع الحقيقي {#Method-6--The-Actual-Overhead-Calculator---Getting-Real}
...
## إغلاق أوامر الإنتاج: الحساب الختامي {#Closing-Production-Orders--The-Final-Reckoning}
...
### متى تُغلق أمراً {#When-to-Close-an-Order}
...
### ماذا يحدث عند الإغلاق {#What-Happens-When-You-Close}
...
## طريقتا الإغلاق {#The-Two-Ways-to-Close}

### الإغلاق العادي: إتمام المهمة {#Normal-Close--Mission-Accomplished}
...
### الإنهاء: الاستسلام {#Terminate--Calling-It-Quits}
...
## تحليل الانحرافات: لعب دور المحقق {#Variance-Analysis--Playing-Detective}
...
### انحرافات المواد {#Material-Variances}
...
### انحرافات الموارد {#Resource-Variances}
...
### قراءة الأدلة {#Reading-the-Clues}
...
## التكلفة لكل دُفعة: حين لا يكفي مقياس واحد للجميع {#Cost-Per-Batch--When-One-Size-Doesnt-Fit-All}
...
### السيناريو 1: أمر بسيط بتكاليف غير مباشرة معيارية {#Scenario-1--Simple-Order-with-Standard-Overhead}
...
### السيناريو 2: تحليل الانحرافات يكشف مشكلات {#Scenario-2--Variance-Analysis-Reveals-Problems}
...
### السيناريو 3: استخدام التكاليف الغير مباشرة الفعلية {#Scenario-3--Using-Actual-Overhead}
...
## الإعدادات التي تتحكم بكل شيء {#Configuration-That-Controls-Everything}

عدة إعدادات تتحكم في آلية عمل التكاليف:

### توجيه أمر الإنتاج (Production Order Term) {#Production-Order-Term}
...
### توجيه إغلاق الأمر (Order Close Term) {#Order-Close-Term}
...
### إعداد نوع التكاليف الغير مباشرة (Overhead Type Configuration) {#Overhead-Type-Configuration}
...
## نصائح لتكلفة فعّالة {#Tips-for-Effective-Costing}
