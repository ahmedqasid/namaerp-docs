...
## تكامل WhatsApp عبر WasenderAPI

لإرسال رسائل WhatsApp من نظام Nama ERP باستخدام [wasenderapi.com](https://wasenderapi.com)، اتبع الخطوات التالية:

---

### خطوات الإعداد

1. **إنشاء جلسة (Session) وربط واتساب**

   * قم بإنشاء حساب على [wasenderapi.com](https://wasenderapi.com) ثم افتح تبويب **Sessions** في لوحة التحكم
   * أنشئ **Session** جديدة وقم بربطها برقم WhatsApp عن طريق مسح رمز QR

2. **نسخ مفتاح API الخاص بالجلسة**

   * لكل جلسة **API Key** خاص بها يظهر في صفحة الجلسة بعد ربط الرقم
   * هذا المفتاح مرتبط بالجلسة فقط؛ حذف الجلسة يلغي صلاحية المفتاح
...
* **مزود الخدمة**: `WasenderAPI`
* **كلمة المرور (Secret)**: مفتاح API الخاص بالجلسة
* **اسم المستخدم (Public ID)**: يُترك فارغًا. لا يستخدم WasenderAPI معرّف Instance، لذلك يكون الحقل معطلًا مع هذا المزود

للإرسال من عدة أرقام، أنشئ جلسة لكل رقم وأضف سطرًا لكل مرسل في جدول **المعرفات العامة حسب المرسل (Public IDs by Sender)** مع وضع مفتاح API الخاص بالجلسة في عمود **Secret**. اترك عمود **Public ID** فارغًا؛ فهو مطلوب فقط مع المزودين الذين يعملون بمعرّفات Instance.

::: tip رسائل الوسائط
اضبط **نوع الوسائط (Media Type)** في رسالة الواتساب على ملف أو صورة أو فيديو أو صوت حتى يصل المرفق إلى المستلم بالشكل المناسب: ملف PDF للفاتورة كمستند، والصورة كصورة داخل المحادثة. وكما هو الحال مع بقية المزودين، يجب أن يكون **رابط الوسائط (Media URL)** متاحًا عبر الإنترنت.
:::

::: warning حدود الإرسال
تسمح الخطط المدفوعة في WasenderAPI بإرسال رسالة واحدة تقريبًا كل 5 ثوانٍ لكل جلسة، بينما تقتصر الحسابات التجريبية على رسالة واحدة في الدقيقة و50 رسالة في اليوم. الرسائل التي تُرسل أسرع من ذلك يرفضها المزود وتظهر كمهام فاشلة تحمل تلميح "retry after" من المزود.
:::

::: tip
يمكن استخدام WasenderAPI أيضًا كمزود للرسائل القصيرة (SMS) من خلال شاشة إعدادات الرسائل القصيرة (مزود الخدمة `WasenderAPI`، كلمة المرور = مفتاح API الخاص بالجلسة)، حيث يتم إرسال الرسائل عبر WhatsApp بدلاً من SMS التقليدي.
:::
...
| **Public ID** | معرّف الـ Instance الخاص بهذا الرقم | نعم، ما عدا Morasalaty وWasenderAPI لأنهما لا يستخدمانه |
