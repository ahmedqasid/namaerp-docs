...
| `CRN` | السجل التجاري | رقم السجل التجاري أو الرقم الوطني الموحد | الجهات والشركات |
| `MOM` | رخصة وزارة الشؤون البلدية | الرقم المميز | حسب الترخيص |
| `MLS` | رخصة وزارة الموارد البشرية | الرقم المميز | حسب الترخيص |
| `700` | الرقم الموحد 700 | الرقم المميز | حسب التسجيل |
...
إن لم تحدّد **نوع هوية المشتري**، يستنتج النظام الكود من الحقل المعبأ: رقم الهوية لعميل **فرد** ← `NAT`، ولعميل **أجنبي** ← `PAS`؛ ورقم السجل التجاري أو الرقم الوطني الموحد أو الرقم المميز ← `CRN`؛ ورقم التسجيل الضريبي ← `TIN`. لا تحتاج لتحديد النوع صراحةً إلا عندما تريد فرض كود معيّن (مثل `MOM` أو `MLS` أو `SAG` أو `GCC` أو `IQA`)، وعندها يُؤخَذ **الرقم المميز** قيمةً للهوية.
:::
