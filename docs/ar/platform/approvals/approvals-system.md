...
3. **تعيين خيارات القرار**
   ...
   - **تصعيد إلى المدير الأعلي (Escalate to Supervisor)**: تسليم الخطوة إلى **المدير الأعلي** للمعتمِد
   - **تصعيد إلى المشرف المباشر (Escalate to Direct Supervisor)**: تسليم الخطوة إلى **المشرف المباشر** للمعتمِد
   - **تصعيد الي موظف بعينه (Escalate to Specific Employee)**: تسليم الخطوة إلى شخص يختاره المعتمِد

#### الخطوات المشروطة (الاختيارية)

ليس من الضروري أن تُنفَّذ كل خطوة موافقة مع كل سجل. أحياناً تريد أن تظهر خطوة ما فقط في ظروف معيّنة — مثلاً "لا تطلب موافقة المدير العام إلا عندما يتجاوز إجمالي الفاتورة 100,000" بينما تتوقف الفواتير الأصغر عند مدير الحسابات. هذا هو الغرض من الخطوات المشروطة.

يمكن أن تحمل كل خطوة موافقة شرطها الخاص عبر حقلين:

- **معايير الخطوة (Step Criteria)**: [تعريف المعايير](/ar/platform/criteria-definitions) محفوظ يُبنى عبر باني المعايير المرئي.
- **استعلام تطبيق الخطوة (Step Apply When Query)**: استعلام SQL يُرجع ما إذا كانت الخطوة تنطبق على السجل.

القاعدة بسيطة:

- الخطوة التي يكون فيها **كل من معايير الخطوة واستعلام تطبيق الخطوة فارغاً** هي خطوة **إلزامية** عادية — تُنفَّذ دائماً.
- الخطوة التي تحتوي على **معايير أو استعلام** تصبح **اختيارية (مشروطة)**: تُنفَّذ فقط عندما ينطبق شرطها على السجل. وعندما لا ينطبق الشرط تُتخطّى الخطوة وينتقل سير العمل مباشرةً إلى الخطوة التالية.
- إذا حدّدت الخطوة **معايير واستعلاماً معاً**، فيجب أن **يتحقق كلاهما** حتى تنطبق الخطوة.

يُقيَّم الشرط لحظة وصول سير العمل إلى الخطوة، بناءً على بيانات السجل في ذلك الوقت.

```sql
-- مثال على خطوة: اطلب هذه الخطوة فقط عندما يتجاوز الإجمالي 100,000
SELECT case when {totalAmount} > 100000 then 1 else 0 end
```

::: tip يُعتبر المستند محفوظاً عندما لا تنطبق آخر خطوة اختيارية
عندما يصل سير العمل إلى آخر خطوة متبقية وكانت هذه الخطوة اختيارية ولم يتحقق شرطها، فلا يبقى شيء بحاجة إلى موافقة — تُعتبر الموافقة مكتملة ويُحفَظ المستند تماماً كما لو تمّت الموافقة على جميع الخطوات. يتيح لك هذا إضافة خطوات موافقة "إضافية" تعمل فقط مع السجلات الأعلى خطورة، بينما تنتهي السجلات العادية بمجرد اكتمال خطواتها الإلزامية.
:::

::: warning يجب أن تنطبق خطوة واحدة على الأقل
لا بد أن يكون لدى الموافقة ما تفعله دائماً. إذا جعلت **كل** الخطوات مشروطة ولم ينطبق **أي** شرط منها على سجل معيّن، فسيمنع النظام الحفظ ويطلب منك إصلاح التعريف — إمّا بجعل خطوة واحدة على الأقل تنطبق على السجل، أو بترك معايير واستعلام إحدى الخطوات فارغة حتى تُنفَّذ دائماً. هذا يمنع دخول السجل في موافقة لا يمكن أن تتقدّم أبداً.
:::
...
| **useEscalateToSupervisor** | استخدام قرار تصعيد إلى المدير الأعلي | Use Escalate To Supervisor | يتحكم في توافر خيار "Escalate to Supervisor" (تصعيد إلى المدير الأعلي) |
| **useEscalateToDirectSupervisor** | استخدام قرار تصعيد إلى المشرف المباشر | Use Escalate To Direct Supervisor | يتحكم في توافر خيار "Escalate to Direct Supervisor" (تصعيد إلى المشرف المباشر) |
...
| **useReturnToSpecificStep** | استخدام قرار الإرجاع لخطوة محددة | Use Return To Specific Step | يتحكم في توافر خيار "Return To A Specific Step" (إرجاع لخطوة محددة) |

جميع هذه الخيارات **مفعَّلة افتراضياً** — فالنظام الجديد يتيح كل القرارات إلى أن تقوم بتعطيل ما لا تريده.

##### طريقتان للتصعيد إلى أعلى

التصعيد يعني تسليم خطوتك إلى شخص آخر بدلاً من البت فيها بنفسك، ويوفر النظام مسارين هرميين لأن سجل الموظف يحتوي على **مديرَين** منفصلين:

| القرار | يقرأ حقل الموظف | المعنى المعتاد |
|---|---|---|
| **تصعيد إلى المدير الأعلي** (Escalate to Supervisor) | **المدير الأعلي** (Supervisor) | المدير الأعلى في التسلسل التنظيمي — الشخص الذي يعلو المسؤول المباشر عن المعتمِد |
| **تصعيد إلى المشرف المباشر** (Escalate to Direct Supervisor) | **المشرف المباشر** (Direct Supervisor) | المسؤول المباشر الذي يتبعه المعتمِد في عمله اليومي |

تخيل طلب شراء موجوداً لدى أمين المخزن. إذا كان المبلغ غير معتاد لكنه ضمن السياق اليومي، يصعّد أمين المخزن الطلب إلى **المشرف المباشر** — أي مدير المخزن الذي يعتمد قرارات المخزون يومياً. أما إذا كان الطلب حساساً بدرجة لا يصح معها أن يبت فيه مدير المخزن، فيصعّده أمين المخزن إلى **المدير الأعلي** ليصل مباشرة إلى مدير العمليات المسجَّل في ذلك الحقل.

في الحالتين يستهدف التصعيد مدير **المعتمِد الذي اتخذ القرار**، وليس مدير من أنشأ المستند. ويصبح الموظف المُصعَّد إليه مرشحاً على **نفس الخطوة** — فسير العمل لا ينتقل إلى الخطوة التالية — وبمجرد أن يتخذ قراره تستكمل الحالة من حيث توقفت.

::: warning يجب أن يكون الحقل المستهدف معبَّأ
يفشل القرار برسالة مثل *"الموظف X ليس له مشرف مباشر"* عندما يكون سجل الموظف الخاص بالمعتمِد خالياً من الحقل الذي يقرأه ذلك القرار. لذا قبل تفعيل أي من القرارين على مستوى النظام، تأكد من أن حقلي المدير الأعلي والمشرف المباشر محدَّثان فعلاً في سجلات الموظفين — فالحقل الفارغ يحوّل القرار إلى طريق مسدود لذلك المعتمِد.
:::

::: tip الأسباب والملحوظات تغطي القرارين معاً
أسباب الموافقة المعلَّم عليها **تستخدم مع التصعيد** (Used With Escalation) تظهر مع قرارات التصعيد الثلاثة جميعاً، كما أن إعدادَي الخطوة **يجب إدخال ملحوظة مع التصعيد** و**يجب إدخال السبب مع التصعيد** يسريان على التصعيد إلى المدير الأعلي وإلى المشرف المباشر على حد سواء. فلا حاجة إلى قائمة أسباب منفصلة لكل مسار تصعيد.
:::
...
| **تصعيد إلى المدير الأعلي** (Escalate to Supervisor) | إحالة إلى المدير الأعلي للمعتمِد | يصبح ذلك المدير مرشحاً على نفس الخطوة |
| **تصعيد إلى المشرف المباشر** (Escalate to Direct Supervisor) | إحالة إلى المشرف المباشر للمعتمِد | يصبح ذلك المشرف مرشحاً على نفس الخطوة |
| **تصعيد الي موظف بعينه** (Escalate to Specific Employee) | إحالة إلى شخص محدد | يوافق الموظف المحدد |
| **إرجاع للخطوة السابقة** (Return To Previous Step) | إعادة الحالة خطوة إلى الوراء | يقرر معتمِد الخطوة السابقة من جديد |
| **إرجاع لخطوة محددة** (Return To A Specific Step) | إعادة الحالة إلى خطوة تسمّيها | يقرر معتمِد تلك الخطوة من جديد |

## الإجراءات في هذه الشاشات

القرارات السابقة هي ما يختاره المعتمِد. وهذه هي الأزرار التي توصله إليها، وهي على شاشات السجلات العادية لا على شاشة تعريف الموافقة نفسها.

- **الموافقة على السجلات المختارة** (Approve Selected Lines) — القرار المجمَّع، ويُعرض في شاشة قائمة أي كيان خاضع للموافقة. أشّر على السطور المعلَّقة واضغطه، فتحسمها نافذة واحدة: **القرار** و**تصعيد إلى** (موظف أو صلاحية أو مجموعة موظفين) و**الرقم السري المؤقت** حيث تطلبه الخطوة و**ملاحظات** للتسبيب و**سبب الموافقة**. ويرفض العمل إن لم تؤشّر على شيء، ويرفضه ثانية إن تركت القرار فارغًا. وقائمة قراراته تحمل ستة من الثمانية: موافقة، ورفض، وإرجاع، وتصعيد إلى المدير الأعلي، وإرجاع للخطوة السابقة، وتصعيد الي موظف بعينه — أما **إرجاع لخطوة محددة وتصعيد إلى المشرف المباشر فلا يتاحان إلا على سجل واحد في كل مرة**، لأن كليهما يقتضي النظر في قائمة خطوات الحالة نفسها أولًا.
- **إلغاء طلب الموافقة** (Revoke Approval Request) — يلغي طلب الموافقة المعلَّق ويخرج السجل من دورة الموافقات. ويعمل على السجل الذي أنت فيه، أو على كل سطر أشّرت عليه في القائمة.
- **نقل الموافقات إلى الموظف المفوض** (Move Approvals To Delegated Employee) — في شاشة **التفويض**. احفظ التفويض وأشّر على السطور، فتنتقل حالات الموافقة المعلَّقة الخاصة بالموظف المفوِّض إلى المفوَّض إليه. وهذا هو الزر الذي يجعل التفويض بأثر رجعي؛ فبدونه لا يلتقط التفويض الجديد إلا الحالات التي تنشأ بعده.
...
- `{escalatelink}` - رابط/زر للتصعيد إلى المدير الأعلي
- `{escalatetodirectsupervisorlink}` - رابط/زر للتصعيد إلى المشرف المباشر
...
**خيار في الإعدادات العامة**: **إضافة خطوات الموافقة في سجل التعديل**
...
- **أنواع القرارات المتتبعة**:
  ...
  - `Return To Previous Step` - عند الإرجاع خطوة واحدة للخلف
  - `Return To A Specific Step` - عند الإرجاع إلى خطوة بعينها
  - `Escalate To Supervisor` - عند التصعيد إلى المدير الأعلي للمعتمِد
  - `Escalate To Direct Supervisor` - عند التصعيد إلى المشرف المباشر للمعتمِد
  - `Escalate To Specific Employee` - عند التصعيد إلى موظف محدد
...
رسائل ترفض العمل على سجل داخل دورة موافقة:

| الرسالة | لماذا | ما تفعله |
|---|---|---|
| «لا يمكن التعديل أثناء الموافقات» · «لا يمكن التعديل أثناء انتظار الموافقة للخطوة {0}» | للسجل دورة موافقة مفتوحة، وأنت لا تملك قدرة التعديل أثناء الموافقة، أو سياسة الخطوة تمنعه. | أنهِ الموافقة أو ألغِها، أو استعمل **السماح بالتعديل أثناء الموافقة** مع السياسة المناسبة للخطوة ومع القدرة. |
| «الموظف {0} لا يمكنه الموافقة على  الخطوة {1} في السجل {2} لذلك لا يمكنه التعديل أثناء انتظار الموافقة» | سياسة الخطوة هي **السماح للمصرح لهم ومن يمكنهم الموافقة**، وهذا الشخص ليس من معتمدي الخطوة المنتظرة حالياً. | دع أحد معتمدي تلك الخطوة يجري التعديل، أو خفّف السياسة إلى **السماح للمصرح لهم**. |
| «السجل بانتظار موافقة. لا يمكن حفظ أي تعديلات إلى أن يتم انهاء الموافقات» | جرت محاولة اعتماد على سجل موافقته ما زالت مفتوحة. والصيغة المنتهية بـ«و لكن يمكنك حفظه كمسودة» تعني أن طريق المسودة ما زال متاحاً. | انتظر القرار، أو احفظ كمسودة حيث تعرض الرسالة ذلك. |
| «السجل بانتظار موافقة. لا يمكن الحذف إلى أن يتم انهاء الموافقات» | حذف سجل له دورة موافقة مفتوحة. | أنهِ الموافقة أو ألغِها أولاً. |
| «السجل {0} خاضع لموافقة حاليا» | جرت محاولة مراجعة أو عملية مشابهة أثناء الدورة. | انتظر انتهاء الموافقة. |
| «لا يمكن للمستخدم الحالي حذف {0} بعد الموافقة» | السجل معتمَد، وملف الأمان لا يسمح لك بحذف السجلات بعد الموافقة. | اطلب ذلك ممن يسمح له ملفه، أو ألغِ السجل بدل حذفه. |
| «يمكن استعمال الرفض مع المستندات و الطلبات فقط - في الملفات قم باستعمال قرار الإرجاع» | ضُغط **رفض** على ملف أساسي. | استعمل **إرجاع** — فالملفات الأساسية تُرجَع ولا تُرفض. |

رسائل ترفض حفظ تعريف الموافقة نفسه:

| الرسالة | لماذا | ما تفعله |
|---|---|---|
| «تعريف سير العمل يجب أن يحتوي على خطوة واحدة على الأقل» | جدول الخطوات فارغ. | أضف خطوة. |
| «يجب تحديد حالة علي الأقل يتم استخدام سير العمل معها (عند الانشاء أو الحذف ... إلخ)» | لم يؤشَّر أي من حالات الاستعمال، فلن يعمل التعريف أبداً. | أشّر واحدة على الأقل: الإنشاء أو التعديل أو الحذف أو غيرها. |
| «مسلسل خطوة مكرر : {0}» | خطوتان تحملان المسلسل نفسه. | غيّر مسلسل إحداهما. |
| «أولوية تعريف سير العمل مكررة مسبقا في تعريف آخر {0}» | تعريف آخر للكيان نفسه يستعمل الأولوية ذاتها، فيصير الترتيب بينهما غير محدد. | امنح هذا التعريف أولوية مختلفة. |
| «سياسة التعديل أثناء الموافقة يجب أن تكون {0} أو فارغة لأن اوبشن التعديل أثناء الموافقة غير مفعل» | اختيرت سياسة تعديل للخطوة بينما الخيار العام **السماح بالتعديل أثناء الموافقة** مغلق. | فعّل الخيار العام، أو أفرغ سياسة الخطوة. |
