...
**نوع تأثير الضرائب على التكلفة (Tax Effect on Cost Type)** `value.taxEffectOnCostsType` — يحدد *من أين* يقرأ النظام قاعدة ما إذا كانت ضرائب المشتريات تُضاف إلى تكلفة الصنف أو تُستبعد منها. وتضم القائمة أربع قيم، ومعرفة أيها أيّ مهمة لأن التسميات طويلة:

- **من الإعدادات** — وهو أيضًا معنى ترك الحقل فارغًا: يُطبَّق مفتاح *عدم إضافة ضرائب المشتريات للتكلفة* الأوحد الموجود أدناه على الضرائب الأربع كلها.
- **1 و 2 من سياسة الصنف و 3 و 4 من التوجيه** — قاعدة مقسَّمة: الضريبتان 1 و 2 تتبعان السياسة الضريبية للصنف، والضريبتان 3 و 4 تتبعان السياسة الضريبية لتوجيه المستند.
- **من السياسة الضريبية بالصنف ثم التوجيه لكل الضرائب** — الضرائب الأربع كلها تتبع السياسة الضريبية للصنف، وتُقرأ سياسة التوجيه بدلًا منها إذا لم يكن للصنف سياسة.
- **من السياسة الضريبية بالتوجيه ثم الصنف لكل الضرائب** — الصورة المعاكسة: تُقرأ سياسة التوجيه أولًا، ولا تُقرأ سياسة الصنف إلا إذا لم يكن للتوجيه سياسة.

والقيم الثلاث الأخيرة هي ما يتيح لأصناف أو أنواع مستندات مختلفة معاملة الضرائب بشكل مختلف بدلًا من مفتاح واحد شامل.
...
**عدم إعادة احتساب تكلفة مردود المبيعات وتوريد الجرد والتوريد غير المكلَّف وقت السحب على المكشوف (Do not Re-Calculate Sales Return and Stock Taking, Uncosted Receipt Cost From Overdraft)** `value.calculateSalesReturnCostFromOverdraft` — مفتاح خاص بمعالجة التكلفة، لكنه **ليس** على هذا التبويب: ستجده في تبويب **الجرد المخزني** بين إعدادات الجرد. وعند تفعيله لا يُعيد النظام حساب تكلفة مردودات المبيعات وسندات توريد الجرد والتوريدات غير المكلَّفة عند استخدامها لتغطية سحب على المكشوف. وهو مشروح بالكامل في [إعدادات الجرد المخزني](./stock-taking-configuration.md).
...
**اعتبار السحب على المكشوف لكل محدِّد تكلفة وليس لكل محدِّد كمية (Consider Overdraft per Cost Dimension not Quantity Dimension)** `value.considerOverdraftPerCostDimensionNotQtyDimension` *(مفعّل في قواعد البيانات المنشأة حديثاً)* — يغيّر درجة التفصيل التي يُقيَّم بها المخزون السالب (السحب على المكشوف) ويُغطّى. عند تفعيله يُقيَّم عند تجميع التكلفة الأوسع؛ وعند إيقافه يُتتبَّع عند أدقّ تفصيل للكمية. وليست له قيمة افتراضية مدمجة: قواعد البيانات المنشأة حديثًا تُزرَع وهو مفعّل، أما التركيبات الأقدم فقد يكون فيها موقَفًا، فاقرأ الشاشة ولا تفترض. مفتاح سلوك تكلفة جوهري — غيّره بتأنٍّ، ويفضّل قبل وجود الحركات.

**استخدام المتوسط الحالي لتغطية التحويل المخزني سحبه على المكشوف بنفسه (Use Current Average for Stock Transfer Self Overdraft Coverage)** `value.useCurrentAverageForStockTransferSelfOverdraftCoverage` *(مفعّل في قواعد البيانات المنشأة حديثاً)* — يعالج الحالة التي يغطّي فيها التحويل سحبه على المكشوف بنفسه (نفس السطر هو الصادر والوارد اللاحق). عند تفعيله يُقيَّم ذلك الجزء من التغطية الذاتية بمتوسط التكلفة الحالي بدلًا من الاستعارة من سطر التوريد. يعمل مع الخيار السابق، وتأتي قواعد البيانات المنشأة حديثًا وهو مفعّل بالفعل.
...
تخبر هذه الجداول النظام من أين يسحب تكلفة الوحدة في الحالات الصعبة. كل سطر يوفّر حتى خمسة مصادر بديلة تُجرَّب بالترتيب، وأول نتيجة غير صفرية تفوز. والمصادر المتاحة تسعة: **تكلفة الخروج في التاريخ**، و**تكلفة الدخول في التاريخ**، و**تكلفة دخول حقيقي في التاريخ** (المشتريات والافتتاحي والتجميع وما شابهها)، و**أخر تكلفة دخول على المطلق**، و**متوسط التكلفة الحالي المطلق**، و**التكلفة القياسية**، و**تكلفة السطر**، و**استعلام** (يشغّل استعلام السطر نفسه)، و**صفر**. ويمكن تحديد نطاق السطور بنوع المستند والمحدِّدات المحاسبية ومحدِّدات الصنف الخاصة.
...
## استعمال محددات الأصناف في تكاليف الاعتمادات والتكاليف الإضافية وتسعير المناقصات (Use Specific Dimensions In LC, Receipt Cost and Tender Prices)

كلما وجب توزيع تكلفة على السطور المستلَمة — تكلفة الاستيراد (الاعتماد المستندي)، أو مستند **تكاليف إضافية على التوريد**، أو توزيع تكلفة التوريد بوجه عام — كان على النظام أولًا أن يقرر أي السطور يُعدّ «الشيء نفسه». وهذه المفاتيح هي التي تبني مفتاح التجميع ذاك. تضمين خاصية يعني أن التكلفة تُتتبَّع وتُوزَّع منفصلةً لكل قيمة لتلك الخاصية؛ واستبعادها يدمج تلك السطور معًا. ويُستعمل المفتاح نفسه في مطابقة سطور المناقصة بطلب سعر البيع، فتغييرٌ هنا يحرّك جانب تسعير المناقصات كما يحرّك جانب التكلفة.
