...
**طريقة العرض الافتراضية للمستندات** `value.info.docDefaultMenuItemBehavior` — والمثل مع عناصر قائمة المستندات. والتركيبات كثيفة الإدخال تضبط المستندات على «سجل جديد» والملفات الأساسية على «مطالعة القوائم» عادةً.
...
**إنشاء مستند إدارة الوثائق في نافذة منبثقة** `value.info.createDMSDocInPopUpWindow` *(مفعل افتراضياً)* — يفتح إجراء «إنشاء مستند إدارة وثائق» في نافذة منبثقة بدل الانتقال بعيداً، فلا يفقد المستخدم السجل الذي كان يعمل عليه.
