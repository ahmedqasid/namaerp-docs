---
# Handwritten homepage — GenNamaDocsIndex intentionally skips regenerating this file
# (see hasHandcraftedHomePage in GenNamaDocsIndex.java)
layout: home
title: Nama ERP Docs
hero:
  name: Nama ERP
  text: دليل الاستخدام والتوثيق
  tagline: أدلة تفصيلية وشروحات فيديو وأسئلة شائعة لجميع وحدات نظام نما — من التثبيت حتى الاحتراف
  image:
    src: /hero.svg
    alt: Nama ERP
  actions:
    - theme: brand
      text: ابدأ من هنا
      link: /ar/getting-started/
    - theme: alt
      text: البحث المتقدم
      link: /ar/full-search
    - theme: alt
      text: شروحات الفيديو
      link: /ar/videos/
features:
  - icon: 🚀
    title: بدء الاستخدام
    details: الحد الأدنى لمتطلبات التشغيل، دليل تثبيت النظام، إعدادات Nama Properties، والمصادقة الثنائية (2FA)
    link: /ar/getting-started/
  - icon: ⚙️
    title: الخصائص العامة للنظام
    details: تعديل الشاشات، الموافقات، الصلاحيات والأمان، مسارات الكيان، التقارير، ذكاء الأعمال (BI)، والتنبيهات
    link: /ar/platform/
  - icon: 📦
    title: الوحدات
    details: الفواتير والضرائب، سلسلة التوريد، التصنيع، الموارد البشرية، نقاط البيع، العقارات، تطبيقات الجوال، والتجارة الإلكترونية
    link: /ar/modules/
  - icon: 🔗
    title: التكاملات الخارجية
    details: واجهة برمجة التطبيقات (API)، سيناريوهات التكامل مع الأنظمة الأخرى، استرجاع الفواتير، وتكامل أجهزة البصمة
    link: /ar/integration/
  - icon: 🏗️
    title: البنية المعمارية
    details: كيف بُني نظام نما وكيف يُنشر ويُؤمَّن — البنية المعمارية للتطبيق والبنية التحتية، لمراجعي تقنية المعلومات والأمان
    link: /ar/architecture/
  - icon: 🛠️
    title: إدارة النظام
    details: حل المشكلات الشائعة، إعادة معالجة الكميات والتكاليف والأستاذ العام، وأدوات قواعد البيانات
    link: /ar/admin/
  - icon: 👨‍💻
    title: مرجع المطور
    details: إرشادات كتابة التوثيق، ضوابط طلبات التطوير، وأسئلة شائعة حول GUI Post Actions
    link: /ar/developer/
  - icon: 🤖
    title: مرجع مسارات الكيان
    details: مرجع مولَّد لكل مسار كيان في النظام، وحدةً وحدة — المدخلات والسلوك وملاحظات الاستخدام (بالإنجليزية فقط)
    link: /entity-flows/
  - icon: 📋
    title: ملاحظات الإصدارات
    details: ما الذي جاء به كل إصدار من إصدارات Nama ERP — خصائص جديدة وتحسينات وإصلاحات، شهرًا بشهر
    link: /ar/release-notes/
---
