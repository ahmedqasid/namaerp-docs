# نظام الموافقات - الدليل الشامل لتعريف الموافقة {#Approvals-System---Approval-Definition-Full-Guide}
...
### تعريف الموافقة (Approval Definition) {#Approval-Definition}
...
### حالة الموافقة (Approval Case) {#Approval-Case}
...
### الوصول إلى تعريفات الموافقة {#Accessing-Approval-Definitions}

انتقل إلى: **الأساسيات > الإعدادات > تعريف موافقه** (Basic > Settings > Approval Definition)

### إنشاء تعريف الموافقة الأول {#Creating-Your-First-Approval-Definition}
...
### خيارات الموافقة الخاصة {#Special-Approval-Options}

#### تطلب التنفيذ (Require Execution) {#Require-Execution}
...
#### التأكيد قبل البدء (Confirm Before Starting) {#Confirm-Before-Starting}
...
#### المراجعة عند الاكتمال (Revise On Completion) {#Revise-On-Completion}
...
#### التعديل أثناء الموافقة (Modify While Under Approval) {#Modify-While-Under-Approval}
...
#### التحكم في التعديل على مستوى الحقل (Field-Level Editing Control) {#Field-Level-Editing-Control}
...
#### أمان OTP (كلمة مرور لمرة واحدة) {#OTP-One-Time-Password-Security}
...
#### تطلب موافقة الجميع (Require All Approvers) {#Require-All-Approvers}
...
### الموافقات الشرطية {#Conditional-Approvals}
...
### قواعد الموافقة (Approval Rules) {#Approval-Rules}
...
#### كيف تعمل قواعد الموافقة {#How-Approval-Rules-Work}
...
#### القواعد المدمجة المتاحة {#Available-Built-in-Rules}
...
#### تهيئة قواعد الموافقة {#Configuring-Approval-Rules}
...
#### استخدام معلومات القاعدة في القوالب {#Using-Rule-Information-in-Templates}
...
### موافقات تجاوز الميزانية (Budget Exceeded Approvals) {#Budget-Exceeded-Approvals}
...
#### كيف تعمل موافقات الميزانية {#How-Budget-Approvals-Work}
...
#### متطلبات الإعداد {#Configuration-Requirements}
...
#### عملية التحقق من الميزانية {#Budget-Validation-Process}
...
#### أبعاد الميزانية المعتبرة {#Budget-Dimensions-Considered}
...
### الأطراف المسؤولة الديناميكية {#Dynamic-Responsible-Parties}
...
### المعتمِدون البدلاء (Alternate Approvers) {#Alternate-Approvers}
...
#### أنواع البدلاء {#Types-of-Alternates}
...
#### الفرق عن الاحتياطي (Fallback) {#Difference-from-Fallback}
...
#### حقول مرجع الموافقة (Approval Reference Fields) {#Approval-Reference-Fields}
...
#### اعتبار تاريخ الطلب كتاريخ الإنشاء (Consider Request Date As Creation Date) {#Consider-Request-Date-As-Creation-Date}
...
#### إعداد ملخص الموافقة (Approval Summary Configuration) {#Approval-Summary-Configuration}
...
## دليل الإعداد خطوة بخطوة {#Step-by-Step-Configuration-Guide}

### الخطوة 1: تعريف الإعدادات الأساسية {#Step-1-Define-Basic-Settings}
...
### الخطوة 2: إعداد خطوات الموافقة {#Step-2-Configure-Approval-Steps}
...
#### إعداد قرار الموافقة العام (Global Approval Decision Configuration) {#Global-Approval-Decision-Configuration}
...
##### خيارات إعداد القرارات المتاحة {#Available-Decision-Configuration-Options}
...
##### تأثير الإعداد {#Configuration-Impact}
...
### الخطوة 3: إعداد الإشعارات {#Step-3-Notification-Configuration}
...
#### إشعارات الرسائل القصيرة {#SMS-Notifications}
...
### الخطوة 4: الميزات المتقدمة {#Step-4-Advanced-Features}

#### مراقبة الحقول الحرجة (Critical Fields Monitoring) {#Critical-Fields-Monitoring}
تتبع تغييرات حقول محددة تستلزم إعادة الموافقة:
- تغييرات المبالغ تجاوزاً للحد المسموح
- تعديلات التواريخ الرئيسية
- تحديثات حقل الحالة

#### التصعيد التلقائي (Auto-escalation) {#Auto-escalation}
...
## سير عمل المستخدم {#User-Workflow}

### لمُنشئي المعاملات {#For-Transaction-Creators}
...
### إجراءات الموافقة المتاحة {#Approval-Actions-Available}
...
#### متغيرات Tempo المتاحة لقوالب الموافقة {#Available-Tempo-Variables-for-Approval-Templates}
...
### الوصول إلى بيانات حالة الموافقة في قوالب الإشعارات {#Accessing-Approval-Case-Data-in-Notification-Templates}
...
#### حقول ApprovalCase المتاحة {#Available-ApprovalCase-Fields}
...
#### حقول ApprovalCaseStep {#ApprovalCaseStep-Fields}
...
#### حقول ApprovalCaseStepCandidate {#ApprovalCaseStepCandidate-Fields}
...
#### أمثلة عملية لقوالب الإشعارات {#Practical-Examples-for-Notification-Templates}
...
#### الموافقات القائمة على الميزانية {#Budget-Based-Approvals}
```
Account Setup:
...
#### الموافقات متعددة المعايير {#Multi-criteria-Approvals}
```
...
#### مراقبة الموافقات {#Monitoring-Approvals}
...
## سجل الإجراءات وسجل المراجعة {#Actions-History-and-Audit-Trail}
...
### السلوك الافتراضي لسجل الإجراءات {#Default-Actions-History-Behavior}
...
### التتبع المحسّن خطوة بخطوة {#Enhanced-Step-by-Step-Tracking}
...
### ميزات الامتثال {#Compliance-Features}
